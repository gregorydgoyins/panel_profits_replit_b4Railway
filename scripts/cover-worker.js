...
// scripts/cover-worker.ts
async function enrichCovers() {
  console.log("\u{1F4F8} COVER WORKER STARTED\n");
  let total = 0;
  const start = Date.now();
  while (true) {
    try {
      const batch = await db.select().from(assets).where(isNull(assets.coverImageUrl)).limit(100);
      if (batch.length === 0) {
      ...
      for (const a of batch) {
        ...
        if (type === "character") {
          ...
          const text2 = encodeURIComponent(a.name.substring(0, 20));
          const color = type === "comic" ? "2d132c/ee4540" : "801336/ee4540";
          url = `https://via.placeholder.com/400x600/${color}?text=${text2}`;
        }
        await db.update(assets).set({ coverImageUrl: url }).where(sql`id = ${a.id}`);
        total++;
      }
      const rate = (total / ((Date.now() - start) / 1e3)).toFixed(1);
