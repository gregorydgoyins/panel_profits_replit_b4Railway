...
async function fastGen() {
  console.log(`🚀 FAST GENERATION: ${TARGET.toLocaleString()} ASSETS\n`);
  ...
  for (let b = 0; b < TARGET / BATCH; b++) {
    ...
    for (let i = 0; i < BATCH; i++) {
      ...
      // UNIQUE SYMBOL using timestamp + random + index
      const uniqueSymbol = `F${Date.now()}${Math.floor(Math.random()*99999)}${idx}`;
      
      aB.push({
        symbol: uniqueSymbol,
        name: `Asset ${Date.now()}-${idx}`,
        type,
    ...
    try {
      ...
      console.error(`❌ Batch ${b}: ${e.message.substring(0, 80)}`);
    }
  }
