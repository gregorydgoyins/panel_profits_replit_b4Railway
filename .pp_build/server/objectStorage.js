...
class ObjectStorageService {
    ...
    async getComicCoverUploadURL(publisher, series, issueNumber, variant = 'regular') {
        const publicDir = this.getPublicObjectSearchPaths()[0];
        const sanitize = (str) => str.toLowerCase().replace(/[^a-z0-9-]/g, '-');
        const sanitizedIssue = sanitize(issueNumber);
        const coverPath = `${publicDir}/covers/${sanitize(publisher)}/${sanitize(series)}/${sanitizedIssue}_${sanitize(variant)}.jpg`;
        const { bucketName, objectName } = parseObjectPath(coverPath);
        return signObjectURL({
            bucketName,
            objectName,
            method: "PUT",
    ...
    async uploadComicCoverFromURL(imageUrl, publisher, series, issueNumber, variant = 'regular') {
        const publicDir = this.getPublicObjectSearchPaths()[0];
        const sanitize = (str) => str.toLowerCase().replace(/[^a-z0-9-]/g, '-');
        const sanitizedIssue = sanitize(issueNumber);
        const coverPath = `${publicDir}/covers/${sanitize(publisher)}/${sanitize(series)}/${sanitizedIssue}_${sanitize(variant)}.jpg`;
        ...
        // Set ACL policy to mark as public
        await (0, objectAcl_1.setObjectAclPolicy)(file, {
            owner: 'system',
            visibility: 'public'
        });
        return `/public-objects/covers/${sanitize(publisher)}/${sanitize(series)}/${sanitizedIssue}_${sanitize(variant)}.jpg`;
    }
}
