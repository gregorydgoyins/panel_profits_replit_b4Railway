...
exports.comicDataService = void 0;
const crypto_1 = __importDefault(require("crypto"));
const tickerFormatter_js_1 = require("../utils/tickerFormatter.js");
const comicFactChecker_js_1 = require("./comicFactChecker.js");
class ComicDataService {
    constructor() {
        this.marvelBaseUrl = 'https://gateway.marvel.com/v1/public';
        this.superHeroBaseUrl = 'https://superheroapi.com/api';
        this.comicVineBaseUrl = 'https://comicvine.gamespot.com/api';
        this.marvelPublicKey = process.env.MARVEL_API_PUBLIC_KEY;
        this.marvelPrivateKey = process.env.MARVEL_API_PRIVATE_KEY;
        this.superHeroToken = process.env.SUPERHERO_API_TOKEN;
        this.comicVineApiKey = process.env.COMIC_VINE_API_KEY;
    }
    generateMarvelAuth() {
    ...
    async fetchMarvelCharacters(limit = 100, offset = 0) {
        try {
    ...
    async fetchMarvelComics(limit = 100, offset = 0) {
        try {
            const auth = this.generateMarvelAuth();
    ...
    async fetchMarvelComicById(comicId) {
        try {
            const auth = this.generateMarvelAuth();
            ...
            if (data.code === 200 && data.data.results.length > 0) {
                ...
                if (yearsOld > 40)
                    estimatedValue *= (20 + Math.random() * 80);
                else if (yearsOld > 20)
                    estimatedValue *= (5 + Math.random() * 15);
                else if (yearsOld > 10)
                    estimatedValue *= (2 + Math.random() * 5);
                else
                    estimatedValue *= (1 + Math.random() * 2);
                ...
                if (isFirstIssue)
                    estimatedValue *= (2 + Math.random() * 3);
                ...
                const tickerSymbol = (0, tickerFormatter_js_1.formatTickerSymbol)('', comicName);
                return {
                    id: comic.id,
                    title: comic.title,
    ...
    async fetchSuperHero(characterName) {
        try {
    ...
    async searchComicVineCharacter(characterName) {
        try {
    ...
    generatePrice(marvelChar, superHero) {
        // Base price calculation based on character popularity/data
        let basePrice = 1000;
        // Factor in comics appeared
        if (marvelChar.comics.available > 100)
            basePrice += 1500;
        else if (marvelChar.comics.available > 50)
            basePrice += 1000;
        else if (marvelChar.comics.available > 20)
            basePrice += 500;
        // Factor in series appeared
        if (marvelChar.series.available > 10)
            basePrice += 800;
        else if (marvelChar.series.available > 5)
            basePrice += 400;
     ...
     */
    async fetchRandomComicCovers(limit = 1) {
        try {
            ...
            const validComics = comics
                ...
                let estimatedValue = printPrice;
                if (yearsOld > 40)
                    estimatedValue *= (20 + Math.random() * 80); // Golden/Silver Age
                else if (yearsOld > 20)
                    estimatedValue *= (5 + Math.random() * 15); // Bronze Age
                else if (yearsOld > 10)
                    estimatedValue *= (2 + Math.random() * 5); // Modern Age
                else
                    estimatedValue *= (1 + Math.random() * 2); // Recent issues
                // CRITICAL: A comic is ONLY a first issue if issueNumber === 1
                // Never check title text - it's unreliable (e.g., "Fantastic Four #101" is NOT a first issue)
                const isFirstIssue = comic.issueNumber === 1;
                if (isFirstIssue)
        ...
        try {
            ...
            const comics = data.data.results
                ...
                return {
                    ...
                    onsaleDate: comic.dates.find((d) => d.type === 'onsaleDate')?.date || null,
                    creators: comic.creators.items.slice(0, 3).map((c) => ({ name: c.name, role: c.role })),
                    pageCount: comic.pageCount,
                    format: comic.format,
                    yearsOld,
                    isFirstIssue,
            ...
            const factCheckResult = await comicFactChecker_js_1.comicFactChecker.verifyComicFacts(comicWithContext);
            comicFactChecker_js_1.comicFactChecker.logFactCheckResults(comicWithContext, factCheckResult);
            ...
            if (Object.keys(factCheckResult.correctedFacts).length > 0) {
                ...
                return comicFactChecker_js_1.comicFactChecker.applyCorrections(comicWithContext, factCheckResult.correctedFacts);
            }
     ...
     */
    async getFallbackComicOfTheDay() {
        try {
    ...
    generateHistoricalContext(comic) {
        const contexts = [
     ...
     */
    async fetchComicVinePublishers(limit = 20) {
        try {
            ...
            return data.results.map((publisher) => ({
                id: publisher.id,
                name: publisher.name,
     ...
     */
    async fetchComicVineCharacters(publisherFilter, limit = 50) {
        try {
            if (!this.comicVineApiKey) {
                console.warn('Comic Vine API key not configured');
                return [];
            ...
            return data.results
                .filter((char) => char.image && !char.image.original_url?.includes('blank'))
                .map((character) => ({
                id: character.id,
                name: character.name,
     ...
     */
    async fetchComicVineVolumes(publisherFilter, limit = 50) {
        try {
            if (!this.comicVineApiKey) {
                console.warn('Comic Vine API key not configured');
                return [];
            ...
            return data.results
                .filter((vol) => vol.image && !vol.image.original_url?.includes('blank'))
                .map((volume) => ({
                id: volume.id,
                name: volume.name,
     ...
     */
    async fetchComicVineIssues(volumeId, limit = 50) {
        try {
            if (!this.comicVineApiKey) {
                console.warn('Comic Vine API key not configured');
                return [];
            ...
            return data.results
                .filter((issue) => issue.image && !issue.image.original_url?.includes('blank'))
                .map((issue) => ({
                id: issue.id,
     ...
     */
    async fetchComicVineCreators(limit = 50) {
        try {
            if (!this.comicVineApiKey) {
                console.warn('Comic Vine API key not configured');
                return [];
            ...
            return data.results
                .filter((person) => person.image && !person.image.original_url?.includes('blank'))
                .map((creator) => ({
                id: creator.id,
                name: creator.name,
...
exports.comicDataService = new ComicDataService();
