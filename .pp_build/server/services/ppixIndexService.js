...
class PPIxIndexService {
    ...
    calculateIndexValue(constituents, indexType) {
        ...
        const weightedConstituents = constituents.map(asset => ({
    ...
    isLegendaryCreator(asset) {
        ...
        const name = asset.name?.toLowerCase() || '';
        return asset.category === 'creator' && legendaryCreators.some(creator => name.includes(creator.toLowerCase()));
    }
    // Identify modern characters for PPIx 100
    isModernCharacter(asset) {
        ...
        const name = asset.name?.toLowerCase() || '';
        return modernCharacters.some(modern => name.includes(modern.toLowerCase()));
    }
    // Check growth potential indicators
    hasGrowthPotential(asset) {
    ...
    hasMediaPresence(asset) {
        ...
        const name = asset.name?.toLowerCase() || '';
        return mediaCharacters.some(media => name.includes(media.toLowerCase()));
    }
    // Get index methodology explanation
    getIndexMethodology(indexType) {
        if (indexType === 'ppix50') {
    ...
    generateIndexComparison(ppix50, ppix100) {
        return {
            ppix50: {
    ...
    calculateCorrelation(index1, index2) {
...
exports.ppixIndexService = new PPIxIndexService();
