...
class MarketPricingService {
    ...
    async getHeritagePrice(identifier, metadata) {
    ...
    async getSignificanceBasedPrice(identifier, metadata) {
        let basePrice = 500;
        if (metadata) {
            ...
            const isMajorCharacter = majorCharacters.some(major => identifier.toLowerCase().includes(major.toLowerCase()) ||
            ...
            const hasMediaAppearance = mediaCharacters.some(media => identifier.toLowerCase().includes(media.toLowerCase()) ||
            ...
            if (metadata.type === 'creator') {
                ...
                const isLegendary = legendaryCreators.some(legend => identifier.toLowerCase().includes(legend.toLowerCase()));
                if (isLegendary) {
                    basePrice *= 6.0;
                }
            }
    ...
    async getSentimentBasedPrice(identifier, metadata) {
        let basePrice = 300;
    ...
    async getMarketPricing(identifier, metadata) {
        const prices = [];
        const sources = [];
    ...
    async generateTradingAssetsWithPricing(limit = 500) {
        const assets = [];
        let processed = 0;
        try {
            ...
            for (let offset = 0; offset < 400 && processed < limit; offset += 50) {
                ...
                const characters = await comicDataService_js_1.comicDataService.fetchMarvelCharacters(50, offset);
                for (const character of characters) {
                    ...
                    try {
                        ...
                        const superHeroData = await comicDataService_js_1.comicDataService.fetchSuperHero(character.name);
                        const asset = {
            ...
            const comics = await comicDataService_js_1.comicDataService.fetchMarvelComics(100, 0);
    ...
    calculateSignificance(marvelChar, superHero, pricing) {
        let significance = 50;
...
exports.marketPricingService = new MarketPricingService();
