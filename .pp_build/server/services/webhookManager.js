...
exports.WebhookManager = exports.IncomingWebhookProcessor = exports.WebhookAnalytics = exports.WebhookDelivery = exports.WebhookSecurity = exports.WebhookQueue = exports.EXTERNAL_EVENTS = exports.PANEL_PROFITS_EVENTS = void 0;
...
class WebhookQueue {
     ...
     */
    async enqueue(webhookId, eventType, payload, priority = 5) {
        const job = {
    ...
    async processJob(job) {
        try {
            ...
            if (job.attempts < job.maxAttempts) {
                ...
                // Log failed delivery
                await WebhookAnalytics.recordFailedDelivery(job.webhookId, job.eventType, error.message);
            }
        }
    }
}
...
class WebhookDelivery {
     ...
     */
    static async deliverWebhook(webhookId, eventType, payload) {
        const webhook = await storage_js_1.storage.getIntegrationWebhook(webhookId);
        ...
        try {
            ...
            const headers = {
                ...
                'X-Panel-Profits-Delivery': crypto_1.default.randomUUID(),
                ...(webhook.headers || {}),
            };
            if (webhook.secretKey) {
                headers['X-Panel-Profits-Signature'] = WebhookSecurity.generateSignature(payloadString, webhook.secretKey);
            }
     ...
     */
    static transformPayload(webhook, eventType, payload) {
        // Apply webhook-specific payload transformation
        if (webhook.payload) {
            const template = webhook.payload;
     ...
     */
    static applyPayloadTemplate(template, context) {
     ...
     */
    static async recordDeliverySuccess(webhook, eventType, responseTime) {
     ...
     */
    static async recordDeliveryFailure(webhook, eventType, errorMessage) {
        // Note: totalTriggers, successfulTriggers, and failedTriggers are computed fields
        // managed by the database. We only update fields that are allowed in InsertIntegrationWebhook.
     ...
     */
    static calculateAverageResponseTime(currentAverage, totalSuccessful, newResponseTime) {
...
WebhookDelivery.DEFAULT_TIMEOUT = 30000; // 30 seconds
WebhookDelivery.USER_AGENT = 'Panel-Profits-Webhooks/1.0';
/**
...
class WebhookAnalytics {
     ...
     */
    static async recordFailedDelivery(webhookId, eventType, errorMessage) {
     ...
     */
    static async getWebhookMetrics(webhookId) {
        const webhook = await storage_js_1.storage.getIntegrationWebhook(webhookId);
        if (!webhook) {
...
class IncomingWebhookProcessor {
     ...
     */
    static async processIncomingWebhook(integrationName, headers, body, rawBody) {
        try {
            ...
            if (integration.encryptedCredentials && signatureHeader) {
                const isValid = await this.verifyIncomingSignature(integration, signatureHeader, rawBody);
                if (!isValid) {
            ...
            const transformedData = externalIntegrationsService_js_1.DataTransformationService.transformFromExternalTool(body, integrationName, eventType);
     ...
     */
    static async findIntegrationByName(name) {
     ...
     */
    static getSignatureHeader(integrationName, headers) {
     ...
     */
    static async verifyIncomingSignature(integration, signature, rawBody) {
        try {
     ...
     */
    static extractEventType(integrationName, headers, body) {
     ...
     */
    static async routeWebhookEvent(integration, eventType, data) {
        // Route to different handlers based on event type
        switch (eventType) {
            case exports.EXTERNAL_EVENTS.WEBFLOW.FORM_SUBMISSION:
                await this.handleFormSubmission(integration, data);
                break;
            case exports.EXTERNAL_EVENTS.FIGMA.FILE_UPDATED:
                await this.handleFigmaFileUpdate(integration, data);
                break;
            case exports.EXTERNAL_EVENTS.ZAPIER.ZAP_TRIGGERED:
     ...
     */
    static async handleFormSubmission(integration, data) {
     ...
     */
    static async handleFigmaFileUpdate(integration, data) {
     ...
     */
    static async handleZapierTrigger(integration, data) {
     ...
     */
    static async logIncomingWebhook(integrationId, eventType, status) {
        console.log(`Incoming webhook: ${integrationId} - ${eventType} - ${status}`);
    }
}
exports.IncomingWebhookProcessor = IncomingWebhookProcessor;
/**
...
class WebhookManager {
     ...
     */
    static async sendWebhook(eventType, payload, userId, integrationFilter) {
        try {
     ...
     */
    static async processIncoming(integrationName, headers, body, rawBody) {
        return await IncomingWebhookProcessor.processIncomingWebhook(integrationName, headers, body, rawBody);
    }
    /**
    ...
    static async createWebhook(webhookData) {
        ...
        return await storage_js_1.storage.createIntegrationWebhook(webhookData);
    }
    /**
     ...
     */
    static async findWebhooksForEvent(eventType, userId, integrationFilter) {
     ...
     */
    static async getAnalytics(webhookId) {
        return await WebhookAnalytics.getWebhookMetrics(webhookId);
    }
}
exports.WebhookManager = WebhookManager;
WebhookManager.queue = new WebhookQueue();
