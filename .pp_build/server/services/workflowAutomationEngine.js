...
exports.workflowEngine = exports.WorkflowAutomationEngine = void 0;
exports.processTrigger = processTrigger;
const storage_js_1 = require("../storage.js");
const webflowIntegration_js_1 = require("./webflowIntegration.js");
const figmaIntegration_js_1 = require("./figmaIntegration.js");
const relumeIntegration_js_1 = require("./relumeIntegration.js");
class WorkflowAutomationEngine {
    ...
    async processTrigger(context) {
        try {
            ...
            const executionPromises = matchingWorkflows.map(workflow => this.executeWorkflow(workflow, context));
     ...
     */
    async findMatchingWorkflows(context) {
        try {
            const allWorkflows = await storage_js_1.storage.getUserWorkflowAutomations(context.userId);
            return allWorkflows.filter((workflow) => {
                if (!workflow.isActive)
                    return false;
                // Check trigger type match
                if (workflow.triggerType !== context.triggerType)
                    return false;
     ...
     */
    evaluateTriggerConditions(triggerConfig, context) {
        try {
            if (!triggerConfig)
                return true;
     ...
     */
    async executeWorkflow(workflow, triggerContext) {
        ...
        try {
            ...
            await storage_js_1.storage.updateWorkflowExecution(executionId, {
     ...
     * Execute workflow actions
     */
    async executeWorkflowActions(workflow, triggerContext, executionId) {
        const errors = [];
     ...
     */
    async executeAction(context, maxRetries = 3) {
    ...
    async executeWebflowCreatePage(context) {
        const config = context.actionConfig;
        const result = await webflowIntegration_js_1.WebflowIntegrationService.generatePortfolioPage(context.integrationId, config.siteId, {
    ...
    async executeWebflowUpdatePortfolio(context) {
        const config = context.actionConfig;
    ...
    async executeWebflowPublishSite(context) {
        const config = context.actionConfig;
    ...
    async executeFigmaExportTokens(context) {
        const config = context.actionConfig;
        const result = await figmaIntegration_js_1.FigmaIntegrationService.extractDesignTokens(context.integrationId, config.fileKey, {
    ...
    async executeFigmaSyncComponents(context) {
        const config = context.actionConfig;
        const components = await figmaIntegration_js_1.FigmaIntegrationService.getFileComponents(context.integrationId, config.fileKey);
        await this.logActionExecution(context, 'figma_components_synced', { componentCount: components.length });
        return components;
    }
    async executeFigmaCreateDesigns(context) {
    ...
    async executeRelumeAnalyzeUsage(context) {
        const config = context.actionConfig;
        const result = await relumeIntegration_js_1.RelumeSitebuilderIntegrationService.analyzeComponentUsage(context.integrationId, config.projectId, config.timeframe || '30d');
        await this.logActionExecution(context, 'relume_usage_analyzed', {
            componentsAnalyzed: result.length
        });
        return result;
    }
    async executeRelumeQualityCheck(context) {
        const config = context.actionConfig;
        const result = await relumeIntegration_js_1.RelumeSitebuilderIntegrationService.performQualityAssessment(context.integrationId, config.projectId);
    ...
    async executeRelumeGeneratePage(context) {
        const config = context.actionConfig;
        const result = await relumeIntegration_js_1.RelumeSitebuilderIntegrationService.generateSitePage(context.integrationId, config.projectId, {
    ...
    async executeWaitDelay(context) {
        const config = context.actionConfig;
     ...
     */
    async logActionExecution(context, eventType, data) {
        try {
            await storage_js_1.storage.createIntegrationSyncLog({
...
exports.workflowEngine = WorkflowAutomationEngine.getInstance();
// Helper function to process triggers from external sources
async function processTrigger(triggerType, triggerData, userId, eventSource) {
    const context = {
        triggerType,
        triggerData,
        userId,
    ...
    return await exports.workflowEngine.processTrigger(context);
}
