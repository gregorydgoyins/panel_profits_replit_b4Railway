...
exports.narrativeEventService = exports.NarrativeEventService = void 0;
...
class NarrativeEventService {
    ...
    async getMarketConditions() {
        try {
            ...
            const assets = await storage_1.storage.getAssets({ type: 'character' });
            const topAssets = assets.slice(0, 10);
            let totalChange = 0;
            let positiveCount = 0;
            let negativeCount = 0;
            for (const asset of topAssets) {
                const marketData = await storage_1.storage.getLatestMarketData(asset.id, '1h');
                if (marketData) {
    ...
    async generateEvent() {
        try {
            ...
            const createdEvent = await storage_1.storage.createNarrativeEvent(event);
            // Apply price impacts
    ...
    async generateChainReaction(parentEvent) {
    ...
    async applyPriceImpacts(event) {
        if (!event.affectedAssets)
            return;
        for (const assetId of event.affectedAssets) {
            try {
                // Get current price
                const currentData = await storage_1.storage.getLatestMarketData(assetId, '1m');
                if (!currentData)
                    continue;
    ...
    // Schedule event deactivation
    scheduleEventDeactivation(event) {
        const timer = setTimeout(async () => {
            try {
                await storage_1.storage.deactivateNarrativeEvent(event.id);
    ...
    async checkActiveEvents() {
        try {
            const activeEvents = await storage_1.storage.getActiveNarrativeEvents();
    ...
    // Helper methods
    selectEventType(marketCondition) {
        ...
        const condition = weights[marketCondition] || weights.sideways;
    ...
    selectSeverity(marketCondition) {
...
exports.NarrativeEventService = NarrativeEventService;
// Export singleton instance
exports.narrativeEventService = new NarrativeEventService();
