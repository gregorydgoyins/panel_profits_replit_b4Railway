...
exports.aiMarketIntelligence = void 0;
...
class AIMarketIntelligenceService {
     ...
     */
    async generatePricePredictions(assets) {
     ...
     */
    async createBeatTheAIChallenge(assets) {
...
Format: Brief analysis with specific price predictions and reasoning.
    ...
    buildMarketAnalysisPrompt(assets) {
...
Focus on actionable insights for serious comic investors.
    ...
    parsePredictionResponse(asset, aiResponse) {
    ...
    parseMarketInsights(aiResponse) {
    ...
    generateFallbackPrediction(asset) {
        const baseChange = this.calculateBaselineChange(asset);
        return {
            assetId: asset.symbol,
            currentPrice: asset.currentPrice,
    ...
    calculateBaselineChange(asset) {
        let changeMultiplier = 0;
        // Age premium (older = more stable growth)
        const age = 2025 - asset.yearPublished;
        if (age >= 80)
            changeMultiplier += 0.08; // Golden Age
        else if (age >= 60)
            changeMultiplier += 0.05; // Silver Age
        else if (age >= 40)
            changeMultiplier += 0.03; // Bronze Age
        // Market cap influence
        if (asset.marketCap >= 1000000)
            changeMultiplier += 0.03; // Blue chip
        else if (asset.marketCap >= 100000)
            changeMultiplier += 0.05; // Growth
        else
            changeMultiplier += 0.08; // Speculative
    ...
    extractReasoningFromAI(aiResponse, asset) {
    ...
    extractMarketFactors(asset) {
        const factors = [];
        if (asset.yearPublished <= 1950)
            factors.push('Golden Age Rarity');
        if (asset.firstAppearance)
            factors.push('First Appearance Premium');
        if (asset.marketCap >= 500000)
            factors.push('Institutional Interest');
        if (asset.publisher === 'Marvel Comics' || asset.publisher === 'DC Comics')
            factors.push('Major Publisher');
        if (asset.grade && parseFloat(asset.grade) >= 9.0)
            factors.push('High Grade Premium');
        return factors.length > 0 ? factors : ['Market Fundamentals'];
    }
    assessRiskLevel(asset) {
        if (asset.marketCap >= 500000 && asset.yearPublished <= 1960)
            return 'LOW';
     ...
     */
    generateIntelligentPrediction(asset) {
        const baseChange = this.calculateBaselineChange(asset);
     ...
     */
    generateIntelligentInsights(marketData) {
        const insights = [];
        // Golden Age analysis
    ...
    calculateVolatility(asset) {
        ...
        if (asset.yearPublished >= 1980)
            return 0.25;
        if (asset.yearPublished >= 1960)
            return 0.15;
...
exports.aiMarketIntelligence = new AIMarketIntelligenceService();
