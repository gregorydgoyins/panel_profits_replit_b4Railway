...
class NormalizationService {
     ...
     */
    fuzzyMatch(entity1, entity2, threshold = 0.85) {
        // Must be same type
        ...
        const nameSimilarity = this.calculateSimilarity(entity1.canonicalName, entity2.canonicalName);
     ...
     */
    createCanonicalEntity(name, entityType, publisher) {
        return {
     ...
     */
    groupSimilarEntities(entities, threshold = 0.85) {
        const groups = [];
        ...
        for (let i = 0; i < entities.length; i++) {
            ...
            processed.add(i);
            const canonical1 = this.createCanonicalEntity(entities[i].entityName, entities[i].entityType, entities[i].publisher);
            for (let j = i + 1; j < entities.length; j++) {
                if (processed.has(j))
                    continue;
                const canonical2 = this.createCanonicalEntity(entities[j].entityName, entities[j].entityType, entities[j].publisher);
    ...
    selectBestName(names) {
        ...
        if (withParenthetical.length === 1)
            return withParenthetical[0];
