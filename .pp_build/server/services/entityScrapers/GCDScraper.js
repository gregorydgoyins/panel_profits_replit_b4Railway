 ...
 */
class GCDScraper extends BaseEntityScraper_1.BaseEntityScraper {
    constructor() {
        const config = {
            sourceName: 'gcd',
        ...
        super(config);
        this.baseUrl = 'https://www.comiccover.org/api/v1';
    }
     ...
     */
    async searchCreators(limit, publisher, offset = 0) {
        const entities = [];
        try {
     ...
     */
    async searchFeatures(limit, publisher, offset = 0) {
        const entities = [];
        try {
     ...
     */
    buildCreatorEntity(creator, awards, relations) {
        const entity = {
     ...
     */
    async fetchCreatorAwards(creatorId) {
        try {
            await this.rateLimit();
     ...
     */
    async fetchCreatorRelations(creatorId) {
        try {
            await this.rateLimit();
     ...
     */
    mapRelationType(gcdType) {
        const lowerType = (gcdType || '').toLowerCase();
        if (lowerType.includes('spouse') || lowerType.includes('married'))
            return 'family';
        if (lowerType.includes('sibling') || lowerType.includes('parent') || lowerType.includes('child'))
            return 'family';
        if (lowerType.includes('mentor') || lowerType.includes('teacher'))
            return 'mentor';
        if (lowerType.includes('student') || lowerType.includes('apprentice'))
            return 'mentor';
        if (lowerType.includes('collabor'))
            return 'ally';
        if (lowerType.includes('rival'))
            return 'rival';
        return 'ally'; // Default
    }
}
exports.GCDScraper = GCDScraper;
