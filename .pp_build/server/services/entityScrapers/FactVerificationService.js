...
exports.factVerificationService = exports.FactVerificationService = void 0;
class FactVerificationService {
    constructor(consensusThreshold = 3) {
        this.consensusThreshold = consensusThreshold;
    }
    /**
     * Verify entity facts across multiple sources
     */
    verifyEntity(sourceDataList) {
     ...
     */
    verifyFirstAppearance(sourceDataList, conflicts) {
        ...
        const grouped = this.groupByKey(firstAppearances, fa => fa.value ? this.normalizeComicTitle(fa.value.comicTitle) : 'unknown');
     ...
     */
    verifyAttributes(sourceDataList, conflicts) {
        // Flatten all attributes with source info
        const allAttributes = sourceDataList.flatMap(sd => (sd.attributes || []).map(attr => ({
            value: attr,
            source: sd.sourceData?.sourceName || 'unknown',
            reliability: sd.sourceData?.sourceReliability || 0.8,
        })));
        ...
        const grouped = this.groupByKey(allAttributes, attr => this.normalizeAttributeName(attr.value.name));
        const verified = [];
        for (const [key, group] of grouped.entries()) {
            ...
            const variantHashes = uniqueVariants.map(v => `${v.description}|${v.level}|${v.isActive}|${v.originType}`);
     ...
     */
    verifyRelationships(sourceDataList, conflicts) {
        const allRelationships = sourceDataList.flatMap(sd => (sd.relationships || []).map(rel => ({
            value: rel,
            source: sd.sourceData?.sourceName || 'unknown',
            reliability: sd.sourceData?.sourceReliability || 0.8,
        })));
        ...
        const grouped = this.groupByKey(allRelationships, rel => `${rel.value.relationshipType}:${this.normalizeEntityName(rel.value.targetEntityName)}`);
        const verified = [];
        for (const [key, group] of grouped.entries()) {
            ...
            // Create hash for each variant to detect duplicates
            const variantHashes = uniqueVariants.map(v => `${v.relationshipType}|${v.targetEntityName}|${v.relationshipSubtype}|${v.strength}|${v.isActive}`);
            const uniqueHashes = [...new Set(variantHashes)];
            if (uniqueHashes.length > 1) {
                conflicts.push({
     ...
     */
    filterByConfidence(result, minConfidence = 0.7) {
        return {
            ...result,
...
exports.FactVerificationService = FactVerificationService;
// Singleton instance
exports.factVerificationService = new FactVerificationService(3);
