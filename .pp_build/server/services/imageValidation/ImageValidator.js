...
exports.ImageStorageManager = exports.ImageValidator = void 0;
...
class ImageValidator {
     ...
     */
    async validateImageUrl(imageUrl, entityName) {
        const result = {
            isValid: false,
            quality: 'rejected',
            issues: [],
     ...
     */
    async validateImageBuffer(buffer, filename, entityName) {
        const result = {
            isValid: false,
            quality: 'rejected',
            issues: [],
     ...
     */
    async validateBatch(imageUrls) {
        const results = new Map();
        // Validate in parallel with rate limiting
        const batchSize = 5;
        for (let i = 0; i < imageUrls.length; i += batchSize) {
            const batch = imageUrls.slice(i, i + batchSize);
            const promises = batch.map(({ url, entityName }) => this.validateImageUrl(url, entityName).then(result => ({ url, result })));
    ...
    // Helper methods
    validateImageUrlPattern(imageUrl, entityName) {
        const issues = [];
    ...
    checkRelevance(imageSource, entityName) {
...
class ImageStorageManager {
     ...
     */
    async storeImage(entityId, entityType, imageUrl, sourceId, entityName) {
    ...
    generateImageId(entityId, sourceId) {
