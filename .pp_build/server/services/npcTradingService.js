...
exports.npcTradingService = exports.NPCTradingService = void 0;
exports.runNPCTradingCycle = runNPCTradingCycle;
...
class NPCTradingService {
    ...
    async runNPCTradingCycle() {
        ...
        try {
            ...
            for (const npcContext of npcsWithContext) {
                try {
                    await this.processNPCTradingDecision(npcContext, marketIntel, newsImpacts, summary);
                }
                catch (error) {
     ...
     */
    async getActiveNPCsWithContext() {
        const activeTraders = await databaseStorage_1.db
            .select()
            .from(schema_1.npcTraders)
            .where((0, drizzle_orm_1.eq)(schema_1.npcTraders.isActive, true));
        const npcsWithContext = [];
        for (const trader of activeTraders) {
            try {
                // Get strategy
                const strategyResults = await databaseStorage_1.db
                    .select()
                    .from(schema_1.npcTraderStrategies)
                    .where((0, drizzle_orm_1.eq)(schema_1.npcTraderStrategies.traderId, trader.id))
                    .limit(1);
                // Get psychology
                const psychologyResults = await databaseStorage_1.db
                    .select()
                    .from(schema_1.npcTraderPsychology)
                    .where((0, drizzle_orm_1.eq)(schema_1.npcTraderPsychology.traderId, trader.id))
                    .limit(1);
                // Get positions
                const positions = await databaseStorage_1.db
                    .select()
                    .from(schema_1.npcTraderPositions)
                    .where((0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.traderId, trader.id));
                ...
                const personality = {
                    ...
                    newsReaction: psychology.newsReaction,
                    lossCutSpeed: psychology.lossCutSpeed,
                };
     ...
     */
    mapHoldingPeriodToFrequency(holdingPeriod) {
        if (holdingPeriod < 1)
            return 'very_high'; // Day traders
        if (holdingPeriod < 7)
            return 'high'; // Swing traders
        if (holdingPeriod < 30)
            return 'medium';
        if (holdingPeriod < 90)
            return 'low';
    ...
    async gatherMarketIntelligence() {
        ...
        try {
            ...
            const allAssets = await databaseStorage_1.db.select().from(schema_1.assets);
    ...
    async analyzeAsset(assetId) {
        try {
            // Get current price
            const currentPriceData = await storage_1.storage.getAssetCurrentPrice(assetId);
            if (!currentPriceData)
                return null;
            const currentPrice = parseFloat(currentPriceData.currentPrice);
            // Get 24h historical data
            const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
            const recentData = await databaseStorage_1.db
                .select()
                .from(schema_1.marketData)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.marketData.assetId, assetId), (0, drizzle_orm_1.gte)(schema_1.marketData.periodStart, oneDayAgo)))
     ...
     */
    calculateHeatScore(volatility, volumeRatio, momentum) {
     ...
     */
    async getRecentNewsImpacts() {
        const impacts = [];
        try {
            const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
            const recentEvents = await databaseStorage_1.db
                .select()
                .from(schema_1.marketEvents)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.marketEvents.isActive, true), (0, drizzle_orm_1.gte)(schema_1.marketEvents.eventDate, oneDayAgo)))
                .orderBy((0, drizzle_orm_1.desc)(schema_1.marketEvents.eventDate));
     ...
     */
    convertImpactToSentiment(impact) {
        switch (impact) {
     ...
     */
    async processNPCTradingDecision(npcContext, marketIntel, newsImpacts, summary) {
        ...
        const targetAsset = this.selectAssetToTrade(preferredAssets, newsImpacts, personality, positions);
        if (!targetAsset) {
        ...
        const shouldTrade = (0, npcPersonalityEngine_1.shouldNPCTrade)(personality, avgSentiment, targetAsset.priceChangePercent, maxSignificance);
        if (!shouldTrade) {
            // Log decision not to trade
            await this.logNPCActivity(trader.id, 'analyze', targetAsset.assetId, null, null, 'Analyzed market conditions but decided not to trade at this time');
            return;
        }
        // Determine trade direction
        const direction = (0, npcPersonalityEngine_1.getNPCTradeDirection)(personality, avgSentiment, targetAsset.priceChangePercent, maxSignificance);
        // Calculate position size
        const availableCapital = parseFloat(trader.currentCapital);
        const positionSize = (0, npcPersonalityEngine_1.calculateNPCPositionSize)(personality, availableCapital, targetAsset.volatility);
        // Calculate quantity
        ...
        // Execute the trade
        await this.executeNPCTrade(npcContext, targetAsset, direction, quantity, summary, `${direction === 'buy' ? 'Bullish' : 'Bearish'} on price action (${targetAsset.priceChangePercent.toFixed(2)}%), sentiment: ${avgSentiment.toFixed(0)}`);
    }
    /**
     * Check if asset matches NPC's preferred types
     */
    async isPreferredAssetType(assetId, preferredTypes) {
        if (preferredTypes.length === 0)
            return true; // No preference means trade anything
        try {
            const asset = await storage_1.storage.getAsset(assetId);
            if (!asset)
                return false;
            return preferredTypes.some(type => asset.type.toLowerCase().includes(type.toLowerCase()) ||
     ...
     */
    selectAssetToTrade(assets, newsImpacts, personality, currentPositions) {
        if (assets.length === 0)
            return null;
        ...
        switch (personality.archetype) {
            ...
            case 'swing_trader':
                ...
                candidates = candidates.filter(a => {
                    if (!a.supportLevel || !a.resistanceLevel)
                        return false;
     ...
     * Execute NPC trade
     */
    async executeNPCTrade(npcContext, targetAsset, direction, quantity, summary, reasoning) {
        ...
        try {
            ...
            if (direction === 'buy') {
                await this.executeNPCBuy(trader.id, targetAsset.assetId, quantity, targetAsset.currentPrice);
            }
            else {
                await this.executeNPCSell(trader.id, targetAsset.assetId, quantity, targetAsset.currentPrice);
            }
            // Update summary
            ...
            // Log the trade
            await this.logNPCActivity(trader.id, direction, targetAsset.assetId, quantity, targetAsset.currentPrice, reasoning);
            ...
            // Log failed attempt
            await this.logNPCActivity(trader.id, 'analyze', targetAsset.assetId, null, null, `Failed to execute ${direction}: ${error}`);
        }
    }
    /**
     * Execute NPC buy order
     */
    async executeNPCBuy(traderId, assetId, quantity, price) {
        const totalCost = quantity * price;
        // Get trader
        const traderResults = await databaseStorage_1.db
            .select()
            .from(schema_1.npcTraders)
            .where((0, drizzle_orm_1.eq)(schema_1.npcTraders.id, traderId))
            .limit(1);
        if (!traderResults[0])
        ...
        const existingPosition = await databaseStorage_1.db
            .select()
            .from(schema_1.npcTraderPositions)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.traderId, traderId), (0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.assetId, assetId)))
            .limit(1);
        if (existingPosition[0]) {
            ...
            await databaseStorage_1.db
                .update(schema_1.npcTraderPositions)
                .set({
                quantity: newQuantity,
                entryPrice: newAvgPrice.toFixed(2)
            })
                .where((0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.id, pos.id));
        }
        else {
            // Create new position
            const newPosition = {
            ...
            await databaseStorage_1.db.insert(schema_1.npcTraderPositions).values(newPosition);
        }
        // Update trader stats
        await databaseStorage_1.db
            .update(schema_1.npcTraders)
            .set({
     ...
     */
    async executeNPCSell(traderId, assetId, quantity, price) {
        ...
        const trader = traderResults[0];
        // Get position
        const positionResults = await databaseStorage_1.db
            .select()
            .from(schema_1.npcTraderPositions)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.traderId, traderId), (0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.assetId, assetId)))
            .limit(1);
        ...
        if (newQuantity <= 0) {
            // Close position
            await databaseStorage_1.db
                .delete(schema_1.npcTraderPositions)
                .where((0, drizzle_orm_1.eq)(schema_1.npcTraderPositions.id, position.id));
        }
        else {
            // Reduce position
            await databaseStorage_1.db
     ...
     */
    async logNPCActivity(traderId, action, assetId, quantity, price, reasoning) {
        try {
            const log = {
                traderId,
                action,
            ...
            await databaseStorage_1.db.insert(schema_1.npcTraderActivityLog).values(log);
        }
        catch (error) {
...
exports.NPCTradingService = NPCTradingService;
// Export singleton instance
exports.npcTradingService = NPCTradingService.getInstance();
/**
 * Main entry point for NPC trading cycle
 */
async function runNPCTradingCycle() {
    return await exports.npcTradingService.runNPCTradingCycle();
}
