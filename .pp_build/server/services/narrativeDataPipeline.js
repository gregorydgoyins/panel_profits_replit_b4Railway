...
exports.narrativeDataPipeline = exports.NarrativeDataPipeline = void 0;
const neon_http_1 = require("drizzle-orm/neon-http");
const serverless_1 = require("@neondatabase/serverless");
const drizzle_orm_1 = require("drizzle-orm");
const narrativeTradingMetricsService_js_1 = require("./narrativeTradingMetricsService.js");
const narrativeMarketIntegration_js_1 = require("./narrativeMarketIntegration.js");
...
class NarrativeDataPipeline {
    ...
    async initialize() {
        ...
        await narrativeTradingMetricsService_js_1.narrativeTradingMetricsService.initializeHouseFinancialProfiles();
     ...
     */
    async processNewStoryBeat(storyBeat) {
        const processingStart = Date.now();
        try {
            console.log(`📖 Processing new story beat: ${storyBeat.beatTitle}`);
            // Add to narrative market integration queue
            narrativeMarketIntegration_js_1.narrativeMarketIntegration.addStoryBeatToQueue(storyBeat);
            // Get affected assets
     ...
     */
    async processEnhancedComicIssue(comicIssue) {
        try {
     ...
     */
    async processCharacterUpdate(character) {
        try {
            console.log(`👤 Processing character update: ${character.name}`);
            // Find character asset
            const characterAssets = await db.select().from(schema_js_1.assets)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'character'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, character.name)));
     ...
     */
    async processMediaPerformanceData(mediaData) {
        try {
     ...
     */
    async updateAllAssetTradingMetrics() {
     ...
     */
    async generateTradingOpportunities() {
        const opportunities = [];
        try {
            ...
            const activeAssets = await db.select()
                ...
                .where((0, drizzle_orm_1.gte)(schema_js_1.narrativeTradingMetrics.lastRecalculation, new Date(Date.now() - 300000))) // Last 5 minutes
                .limit(50);
     ...
     */
    async processNarrativeTimeline(timeline) {
        try {
    ...
    async generateAssetMarketInsights(assetId) {
        try {
            const insights = await narrativeTradingMetricsService_js_1.narrativeTradingMetricsService.generateNarrativeMarketInsights(assetId);
    ...
    async getAffectedAssetsByStoryBeat(storyBeat) {
        ...
        try {
            ...
            if (storyBeat.timelineId) {
                const timeline = await db.select().from(schema_js_1.narrativeTimelines)
                    .where((0, drizzle_orm_1.eq)(schema_js_1.narrativeTimelines.id, storyBeat.timelineId))
                    .limit(1);
                if (timeline[0]) {
                    // Find assets by timeline name or universe
                    const relatedAssets = await db.select().from(schema_js_1.assets)
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'character')
            ...
            if (storyBeat.primaryEntities && Array.isArray(storyBeat.primaryEntities)) {
                for (const entityName of storyBeat.primaryEntities) {
                    const entityAssets = await db.select().from(schema_js_1.assets)
                        .where((0, drizzle_orm_1.eq)(schema_js_1.assets.name, entityName));
    ...
    async extractTradingSignalsFromComic(comicIssue) {
        // Extract narrative signals that affect trading
        const signals = {
            volatilitySignals: [],
            momentumSignals: [],
            sentimentSignals: [],
            culturalImpactSignals: []
        };
    ...
    async getRelatedAssetsByComic(comicIssue) {
        const relatedAssets = [];
        try {
            ...
            if (comicIssue.firstAppearances && Array.isArray(comicIssue.firstAppearances)) {
                for (const characterName of comicIssue.firstAppearances) {
                    const characterAssets = await db.select().from(schema_js_1.assets)
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'character'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, characterName)));
            ...
            if (comicIssue.writers && Array.isArray(comicIssue.writers)) {
                for (const writerName of comicIssue.writers) {
                    const writerAssets = await db.select().from(schema_js_1.assets)
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'creator'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, writerName)));
            ...
            if (comicIssue.pencilers && Array.isArray(comicIssue.pencilers)) {
                for (const artistName of comicIssue.pencilers) {
                    const artistAssets = await db.select().from(schema_js_1.assets)
                        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'creator'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, artistName)));
                    relatedAssets.push(...artistAssets);
                }
            }
            // Series/comic assets
            const seriesAssets = await db.select().from(schema_js_1.assets)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'comic'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, comicIssue.comicSeries)));
    ...
    async applyTradingSignalsToAsset(assetId, signals) {
        try {
            // Get current metrics
            ...
            const updatedMythicVolatility = Math.min(parseFloat(currentMetrics[0].mythicVolatilityScore) + volatilityAdjustment, 10.0);
            const updatedNarrativeMomentum = Math.min(Math.max(parseFloat(currentMetrics[0].narrativeMomentumScore) + momentumAdjustment, -5.0), 5.0);
            await db.update(schema_js_1.narrativeTradingMetrics)
                .set({
    ...
    async processVisualStorytellingData(comicIssue) {
    ...
    async processPowerLevelChanges(assetId, character) {
        try {
            const currentMetrics = await db.select()
                .from(schema_js_1.narrativeTradingMetrics)
                .where((0, drizzle_orm_1.eq)(schema_js_1.narrativeTradingMetrics.assetId, assetId))
    ...
    async updateHouseAffiliations(assetId, character) {
        try {
    ...
    async getRelatedAssetsByMedia(mediaData) {
        const relatedAssets = [];
        try {
            ...
            if (mediaData.characterFamily) {
                const franchiseAssets = await db.select().from(schema_js_1.assets)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'comic'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, mediaData.characterFamily)));
                relatedAssets.push(...franchiseAssets);
            }
            // Find related assets by film title
            const titleAssets = await db.select().from(schema_js_1.assets)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_js_1.assets.type, 'comic'), (0, drizzle_orm_1.eq)(schema_js_1.assets.name, mediaData.filmTitle)));
    ...
    async calculateMediaMomentumBoost(mediaData) {
    ...
    async applyMediaMomentumBoost(assetId, boost) {
        try {
            const currentMetrics = await db.select()
                .from(schema_js_1.narrativeTradingMetrics)
                .where((0, drizzle_orm_1.eq)(schema_js_1.narrativeTradingMetrics.assetId, assetId))
            ...
            const updatedMediaBoost = Math.min(currentBoost + boost, 2.0 // Cap at 200%
            );
            await db.update(schema_js_1.narrativeTradingMetrics)
                .set({
    ...
    async performInitialBulkUpdate() {
    ...
    classifyEventType(event) {
        ...
        if (eventLower.includes('death') || eventLower.includes('killed'))
            return 'character_death';
        if (eventLower.includes('resurrection') || eventLower.includes('returned'))
            return 'resurrection';
        if (eventLower.includes('power') && (eventLower.includes('gain') || eventLower.includes('upgrade')))
            return 'power_upgrade';
        if (eventLower.includes('reveal') || eventLower.includes('identity'))
            return 'identity_reveal';
        if (eventLower.includes('cosmic') || eventLower.includes('universal'))
            return 'cosmic_event';
        return 'general_event';
    }
    calculateEventImpact(event, eventType) {
        ...
        return impactMultipliers[eventType] || 0.2;
    }
    async determineCharacterHouseAffiliation(character) {
        ...
        for (const [houseId, keywords] of Object.entries(houseKeywords)) {
            ...
            if (houseId === 'wisdom' && intelligence >= 8)
                score += 2;
            if (houseId === 'power' && strength >= 8)
                score += 2;
            if (houseId === 'time' && speed >= 8)
                score += 2;
    ...
    async evaluateTradingOpportunity(metrics) {
    ...
    generateRecommendation(volatility, momentum) {
        if (volatility > 0.2)
            return 'High volatility - consider protective strategies';
        if (momentum > 2.0)
            return 'Strong positive momentum - potential buying opportunity';
        if (momentum < -2.0)
            return 'Negative momentum - consider selling or shorting';
        return 'Moderate activity - monitor for changes';
    }
    async getRelatedAssetsByTimeline(timeline) {
        const assets = [];
        try {
            ...
            if (timeline.scope === 'character' && timeline.universe) {
                const characterAssets = await db.select().from(assets)
                    .where((0, drizzle_orm_1.eq)(assets.type, 'character'));
            ...
            const timelineAssets = await db.select().from(assets)
                .where((0, drizzle_orm_1.eq)(assets.name, timeline.timelineName));
    ...
    async determineStoryArcPhase(timeline) {
        // Determine phase based on timeline status and type
        if (timeline.timelineStatus === 'active')
            return 'rising_action';
        if (timeline.timelineStatus === 'completed')
            return 'resolution';
        if (timeline.timelineType === 'character_arc')
            return 'climax';
        return 'rising_action'; // Default phase
    }
    async getPhaseVolatilityMultipliers(phase) {
        const multipliers = {
        ...
        return multipliers[phase] || multipliers.rising_action;
    }
    async applyStoryArcAdjustments(assetId, phase, multipliers) {
        try {
            await db.update(schema_js_1.narrativeTradingMetrics)
                .set({
                storyArcPhase: phase,
    ...
    async detectSignificantNarrativeMovements() {
    ...
    async createMarketAlert(movement) {
    ...
    extractIntensitySignals(panelAnalysis) {
    ...
    async applyIntensitySignals(assetId, signals) {
        ...
        try {
            ...
            const currentMetrics = await db.select()
                ...
                .limit(1);
            if (currentMetrics.length > 0) {
                const updatedVolatility = Math.min(parseFloat(currentMetrics[0].mythicVolatilityScore) + intensityBoost, 10.0);
                await db.update(schema_js_1.narrativeTradingMetrics)
                    .set({
...
exports.NarrativeDataPipeline = NarrativeDataPipeline;
// Export singleton instance
exports.narrativeDataPipeline = new NarrativeDataPipeline();
