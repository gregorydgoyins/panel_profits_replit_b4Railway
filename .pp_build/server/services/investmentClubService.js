...
exports.investmentClubService = exports.InvestmentClubService = void 0;
const storage_js_1 = require("../storage.js");
const notificationService_js_1 = require("./notificationService.js");
const orderMatchingEngine_js_1 = require("./orderMatchingEngine.js");
class InvestmentClubService {
     ...
     */
    async checkClubEligibility(userId, memberUserIds) {
        try {
            const user = await storage_js_1.storage.getUser(userId);
            if (!user) {
                return { eligible: false, reason: 'User not found' };
            }
            const userProgress = await storage_js_1.storage.getUserPathwayProgress(userId);
            ...
            const currentLevel = await storage_js_1.storage.getCareerPathwayLevel(userProgress.currentLevelId);
            if (!currentLevel) {
                return { eligible: false, reason: 'Career level not found' };
            }
            const officeManagerLevels = ['tier2', 'tier3', 'tier4'];
            const isOfficeManagerOrHigher = currentLevel.pathway === 'family_office' && officeManagerLevels.includes(currentLevel.level);
            ...
            for (const memberId of allMemberIds) {
                const member = await storage_js_1.storage.getUser(memberId);
     ...
     */
    async createClub(ownerId, clubData, initialMembers = []) {
        try {
            ...
            const club = await storage_js_1.storage.createInvestmentClub({
                ...clubData,
                ownerId,
                status: 'active',
                monthlyReturns: [],
            ...
            const portfolio = await storage_js_1.storage.createPortfolio({
                userId: ownerId,
            ...
            await storage_js_1.storage.createClubPortfolio({
                clubId: club.id,
            ...
            await storage_js_1.storage.createClubMembership({
                clubId: club.id,
                userId: ownerId,
                role: 'owner',
                sharePercentage,
            ...
            for (const memberId of initialMembers) {
                ...
                await notificationService_js_1.notificationService.sendNotification({
                    userId: memberId,
                    type: 'club_invite',
            ...
            await storage_js_1.storage.createClubActivityLog({
                clubId: club.id,
    ...
    async inviteMember(clubId, inviterId, inviteeId) {
        try {
            const club = await storage_js_1.storage.getInvestmentClub(clubId);
            if (!club)
                throw new Error('Club not found');
            const inviterMembership = await storage_js_1.storage.getClubMembership(clubId, inviterId);
            if (!inviterMembership || !['owner', 'admin'].includes(inviterMembership.role)) {
                throw new Error('Only club owner or admin can invite members');
            }
            const existingMembership = await storage_js_1.storage.getClubMembership(clubId, inviteeId);
            if (existingMembership) {
                throw new Error('User is already a member or has pending invitation');
            }
            const activeMembers = await storage_js_1.storage.getClubMemberships(clubId, 'active');
     ...
     */
    async acceptInvitation(clubId, userId) {
        try {
            const membership = await storage_js_1.storage.getClubMembership(clubId, userId);
            if (!membership || membership.status !== 'pending') {
                throw new Error('No pending invitation found');
            }
            await storage_js_1.storage.updateClubMembership(membership.id, { status: 'active' });
            const activeMembers = await storage_js_1.storage.getClubMemberships(clubId, 'active');
     ...
     */
    async removeMember(clubId, removerId, memberIdToRemove) {
        try {
            const removerMembership = await storage_js_1.storage.getClubMembership(clubId, removerId);
            if (!removerMembership || !['owner', 'admin'].includes(removerMembership.role)) {
                throw new Error('Only owner or admin can remove members');
            }
            const membershipToRemove = await storage_js_1.storage.getClubMembership(clubId, memberIdToRemove);
            if (!membershipToRemove) {
                throw new Error('Member not found');
            }
            await storage_js_1.storage.updateClubMembership(membershipToRemove.id, {
                status: 'removed',
                leftAt: new Date()
            });
            const activeMembers = await storage_js_1.storage.getClubMemberships(clubId, 'active');
    ...
    async leaveClub(clubId, userId) {
        try {
            ...
            await storage_js_1.storage.updateClubMembership(membership.id, {
                status: 'left',
                leftAt: new Date()
            });
            const activeMembers = await storage_js_1.storage.getClubMemberships(clubId, 'active');
     ...
     */
    async recalculateSharePercentages(clubId, activeMembers) {
        const sharePercentage = (100 / activeMembers.length).toFixed(2);
        for (const member of activeMembers) {
            await storage_js_1.storage.updateClubMembership(member.id, { sharePercentage });
        }
    }
    /**
     * Create a proposal
     */
    async createProposal(clubId, proposerId, proposalData) {
        try {
            const membership = await storage_js_1.storage.getClubMembership(clubId, proposerId);
            ...
            const proposal = await storage_js_1.storage.createClubProposal({
                ...proposalData,
                clubId,
                proposerId,
                votesFor: 0,
     ...
     */
    async castVote(proposalId, userId, vote) {
        try {
            const proposal = await storage_js_1.storage.getClubProposal(proposalId);
            if (!proposal)
            ...
            if (proposal.expiresAt && new Date() > proposal.expiresAt) {
                await storage_js_1.storage.updateClubProposal(proposalId, { status: 'expired' });
                throw new Error('Proposal has expired');
            }
            const membership = await storage_js_1.storage.getClubMembership(proposal.clubId, userId);
            if (!membership || membership.status !== 'active') {
                throw new Error('Only active members can vote');
            }
            const existingVote = await storage_js_1.storage.getClubVote(proposalId, userId);
            if (existingVote) {
                throw new Error('Already voted on this proposal');
            }
            await storage_js_1.storage.createClubVote({
                proposalId,
                userId,
                vote
            });
            ...
            await storage_js_1.storage.updateClubProposal(proposalId, {
    ...
    async executeProposal(proposalId) {
        try {
            ...
            const clubPortfolio = await storage_js_1.storage.getClubPortfolioByClubId(proposal.clubId);
            if (!clubPortfolio) {
                return { success: false, error: 'Club portfolio not found' };
            }
            const club = await storage_js_1.storage.getInvestmentClub(proposal.clubId);
            if (!club) {
            ...
            if (proposal.proposalType === 'buy' || proposal.proposalType === 'sell') {
                ...
                const order = await storage_js_1.storage.createOrder({
                    userId: club.ownerId,
                ...
                await orderMatchingEngine_js_1.orderMatchingEngine.processOrders();
            ...
            const activeMembers = await storage_js_1.storage.getClubMemberships(proposal.clubId, 'active');
            for (const member of activeMembers) {
                await notificationService_js_1.notificationService.sendNotification({
                    userId: member.userId,
     ...
     */
    async calculatePerformanceMetrics(clubId) {
        try {
            const club = await storage_js_1.storage.getInvestmentClub(clubId);
            if (!club)
                return null;
            const clubPortfolio = await storage_js_1.storage.getClubPortfolioByClubId(clubId);
            if (!clubPortfolio)
                return null;
     ...
     * Auto-dissolve club
     */
    async autoDissolveClub(clubId, reason) {
        try {
            const club = await storage_js_1.storage.getInvestmentClub(clubId);
            if (!club)
                throw new Error('Club not found');
            const clubPortfolio = await storage_js_1.storage.getClubPortfolioByClubId(clubId);
            if (clubPortfolio) {
                const holdings = await storage_js_1.storage.getPortfolioHoldings(clubPortfolio.portfolioId);
                for (const holding of holdings) {
                    await storage_js_1.storage.createOrder({
                        userId: club.ownerId,
                        portfolioId: clubPortfolio.portfolioId,
            ...
            const activeMembers = await storage_js_1.storage.getClubMemberships(clubId, 'active');
            const portfolio = await storage_js_1.storage.getPortfolio(clubPortfolio?.portfolioId || '');
            if (portfolio) {
            ...
            await storage_js_1.storage.updateInvestmentClub(clubId, {
                status: 'dissolved',
     ...
     */
    async dissolveClub(clubId, ownerId) {
        try {
            const club = await storage_js_1.storage.getInvestmentClub(clubId);
            if (!club)
                throw new Error('Club not found');
...
exports.InvestmentClubService = InvestmentClubService;
exports.investmentClubService = InvestmentClubService.getInstance();
