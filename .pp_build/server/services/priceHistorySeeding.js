...
async function seedPriceHistory() {
    ...
    try {
        ...
        const prices = await storage_1.storage.getAllAssetCurrentPrices();
        const assetPrices = assets.map((asset) => {
            const price = prices.find((p) => p.assetId === asset.id);
            return {
                id: asset.id,
                symbol: asset.symbol,
                currentPrice: price ? parseFloat(price.currentPrice) : 100
            };
        }).filter((ap) => ap.currentPrice > 0);
        ...
        for (const assetPrice of assetPrices) {
            ...
            await storage_1.storage.createPriceHistoryBatch(assetRecords);
