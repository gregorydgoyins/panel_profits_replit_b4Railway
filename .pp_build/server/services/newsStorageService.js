...
exports.newsStorageService = exports.NewsStorageService = void 0;
...
class NewsStorageService {
     ...
     */
    async fetchFromNewsDataIO(maxResults = 50) {
        if (!NEWSDATA_API_KEY) {
    ...
    async storeArticle(article) {
        try {
            ...
            const existing = await databaseStorage_1.db
                .select()
                .from(schema_1.newsArticles)
                .where((0, drizzle_orm_1.sql) `${schema_1.newsArticles.headline} = ${article.headline} AND ${schema_1.newsArticles.sourceOrganization} = ${article.sourceOrganization}`)
                .limit(1);
            if (existing.length > 0) {
                return { stored: false, id: existing[0].id };
            }
            // Extract related asset symbols from headline/summary
            const affectedAssets = await this.extractRelatedAssets(article.headline + ' ' + article.summary);
            const now = new Date();
            ...
            // Store article
            const [newArticle] = await databaseStorage_1.db
                .insert(schema_1.newsArticles)
                .values({
                ...
                .returning({ id: schema_1.newsArticles.id });
     ...
     */
    async extractRelatedAssets(text) {
        try {
            const keywords = [
            ...
            for (const keyword of keywords) {
                if (lowerText.includes(keyword)) {
                    const assets = await databaseStorage_1.db
                        .select({ symbol: schema_1.assets.symbol })
                        .from(schema_1.assets)
                        .where((0, drizzle_orm_1.eq)(schema_1.assets.type, 'character'))
                        .limit(100);
                    const matching = assets.find(a => a.symbol.toLowerCase().includes(keyword.replace(/[^a-z]/g, '')));
                    if (matching) {
    ...
    async getRecentNews(limit = 50) {
        try {
            const articles = await databaseStorage_1.db
                .select()
                .from(schema_1.newsArticles)
                .orderBy((0, drizzle_orm_1.desc)(schema_1.newsArticles.publishTime))
                .limit(limit);
            return articles;
        }
        catch (error) {
     ...
     */
    async getNewsByAsset(symbol, limit = 20) {
        try {
            const articles = await databaseStorage_1.db
                .select()
                .from(schema_1.newsArticles)
                .orderBy((0, drizzle_orm_1.desc)(schema_1.newsArticles.publishTime))
                .limit(1000);
            const filtered = articles.filter(article => article.affectedAssets && article.affectedAssets.includes(symbol));
...
exports.newsStorageService = new NewsStorageService();
