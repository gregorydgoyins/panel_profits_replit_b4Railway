...
const FandomWikiScraper_js_1 = require("./services/entityScrapers/FandomWikiScraper.js");
async function testWiki(wikiKey) {
    const config = FandomWikiScraper_js_1.FANDOM_WIKIS[wikiKey];
    console.log(`\n${'='.repeat(70)}`);
    console.log(`Testing ${config.publisher} Wiki (${wikiKey})`);
    console.log(`Base URL: ${config.baseUrl}`);
    console.log(`${'='.repeat(70)}`);
    try {
        const scraper = new FandomWikiScraper_js_1.FandomWikiScraper(wikiKey);
...
async function main() {
    ...
    for (const [wiki, count] of Object.entries(results)) {
        const config = FandomWikiScraper_js_1.FANDOM_WIKIS[wiki];
