...
function testLargeScale() {
    ...
    const testSize = [1000000, 5000000, 10000000];
    for (const n of testSize) {
        const probability = (n * n) / (2 * symbolSpace);
        const percentage = (probability * 100).toFixed(6);
        console.log(`  For ${(n / 1000000).toFixed(0)}M assets: ${percentage}% collision probability`);
    }
    console.log('\n' + '='.repeat(70));
    if (allLengthsCorrect && collisions === 0) {
        ...
        if (!allLengthsCorrect)
            console.log('   • Variable suffix lengths');
        if (collisions > 0)
