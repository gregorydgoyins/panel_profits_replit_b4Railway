...
async function testExpansionPipeline() {
    ...
    try {
        ...
        const result = await pineconeAssetExpansion_1.pineconeAssetExpansion.expandAssetDatabase(10);
        if (!result.success) {
        ...
        const spiderMan = result.assets.characterAssets.find(a => a.name.toLowerCase().includes('spider-man') || a.name.toLowerCase().includes('spiderman'));
        const milesSpider = result.assets.characterAssets.find(a => a.name.toLowerCase().includes('miles morales'));
        const stanLee = result.assets.creatorAssets.find(a => a.name.toLowerCase().includes('stan lee'));
