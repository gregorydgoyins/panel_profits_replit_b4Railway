...
async function main() {
    ...
    try {
        const chars = await marvelExpansionService_1.marvelExpansionService.expandCharacters(100, 50);
    ...
    try {
        const comics = await marvelExpansionService_1.marvelExpansionService.expandComics(200, 50);
    ...
    try {
        const creators = await marvelExpansionService_1.marvelExpansionService.expandCreators(100, 30);
    ...
    for (const feed of workingFeeds) {
        try {
            ...
            const articles = parsedFeed.items.slice(0, 10).map((item) => ({
