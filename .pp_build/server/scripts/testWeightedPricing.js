...
const priceChartingService_js_1 = require("../services/priceChartingService.js");
async function testPricingFormula() {
    ...
    for (const creator of creators) {
        const price = await priceChartingService_js_1.priceChartingService.getPriceForCreator(creator);
    ...
    for (const character of characters) {
        const price = await priceChartingService_js_1.priceChartingService.getPriceForCharacter(character);
        const status = price >= 50 && price <= 6000 ? '✅' : '❌';
