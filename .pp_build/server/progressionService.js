...
class ProgressionService {
    ...
    async addComicToCollection(userId, variantId, collectionData) {
        try {
    ...
    async recalculateUserProgression(userId) {
        try {
    ...
    async calculateCollectionStats(collections) {
        const stats = {
            totalValue: 0,
            totalIssues: 0,
            totalVariants: 0,
            ...
            creatorMilestones: 0,
            iconicSplashPages: 0,
            tradingToolsUnlocked: []
        };
        for (const collection of collections) {
            if (!collection.contributesToProgression)
                continue;
            // Get variant details
    ...
    getProgressionTitle(tier, stats) {
        ...
        return titles[tier] || "Rookie Collector";
    }
    // ===== HOUSE PROGRESSION =====
    async updateHouseProgression(userId, houseId, experienceGained) {
        try {
    ...
    async checkAndAwardAchievements(userId) {
        try {
            ...
            for (const achievement of availableAchievements) {
                ...
                const meetsRequirements = await this.checkAchievementRequirements(userId, achievement, collections, progressionStatus);
    ...
    async checkAchievementRequirements(userId, achievement, collections, progressionStatus) {
        if (!progressionStatus)
            return false;
    ...
    checkCountRequirement(achievement, collections, progressionStatus) {
    ...
    checkValueRequirement(achievement, progressionStatus) {
        if (!achievement.requiredValue)
            return false;
        return Number(progressionStatus.totalCollectionValue) >= Number(achievement.requiredValue);
    }
    checkRarityRequirement(achievement, collections) {
        if (!achievement.requiredRarity)
            return false;
        const rarityMap = {
    ...
    async checkSpecificIssuesRequirement(achievement, collections) {
        if (!achievement.specificRequirements)
            return false;
        const requirements = achievement.specificRequirements;
        if (!requirements.requiredVariants)
            return false;
        const ownedVariantIds = collections.map(c => c.variantId);
        return requirements.requiredVariants.every((variantId) => ownedVariantIds.includes(variantId));
    }
    checkStorylineRequirement(achievement, collections) {
        // Implementation for storyline completion checks
        return true; // Simplified for now
    }
    // ===== TRADING TOOL UNLOCKS =====
    async checkAndUnlockTradingTools(userId) {
        try {
            const progressionStatus = await this.storage.getUserProgressionStatus(userId);
            if (!progressionStatus)
                return [];
    ...
    hasRequiredVariantRarity(progressionStatus, requiredRarity) {
    ...
    async createCollectionChallenge(challengeData) {
        try {
    ...
    // ===== HELPER METHODS =====
    async updateProgressionAfterNewAcquisition(userId, variant, collection) {
        // Award XP to relevant houses based on variant's house relevance
        if (variant.houseRelevance) {
            const houseRelevance = variant.houseRelevance;
    ...
    async awardLevelUpRewards(userId, houseId, newLevel) {
        try {
    ...
    async applyAchievementRewards(userId, achievement) {
        try {
            ...
            if (achievement.houseProgressionBonus) {
                const bonuses = achievement.houseProgressionBonus;
    ...
    // ===== PUBLIC API METHODS =====
    async getUserProgressionDashboard(userId) {
        try {
            const [progressionStatus, collections, achievements, houseProgressions, tradingUnlocks, activeChallenges] = await Promise.all([
    ...
    async getProgressionLeaderboard(category = 'overall', timeframe = 'all_time') {
        try {
