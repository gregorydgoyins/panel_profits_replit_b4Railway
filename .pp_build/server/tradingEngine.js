...
exports.TradingFirmEngine = exports.InformationTierManager = exports.NpcTradingEngine = exports.ImfVaultingEngine = exports.MarketHoursManager = exports.OptionsCalculator = void 0;
...
class OptionsCalculator {
     ...
     */
    static calculateD1(S, K, r, T, sigma) {
     ...
     */
    static calculateOptionPricing(underlyingPrice, strikePrice, timeToExpiration, // in years
    riskFreeRate = 0.05, // 5% default
     ...
     */
    static calculateImpliedVolatility(marketPrice, underlyingPrice, strikePrice, timeToExpiration, riskFreeRate = 0.05, optionType, tolerance = 0.0001, maxIterations = 100) {
        let sigma = 0.3; // Initial guess: 30% volatility
        for (let i = 0; i < maxIterations; i++) {
            const pricing = this.calculateOptionPricing(underlyingPrice, strikePrice, timeToExpiration, riskFreeRate, sigma, optionType);
...
exports.OptionsCalculator = OptionsCalculator;
// Market Hours Manager
class MarketHoursManager {
    /**
     * Check if a specific market is currently open
     */
    static isMarketOpen(marketHours) {
        const now = new Date();
     ...
     */
    static getNextMarketOpen(marketHours) {
        const now = new Date();
        const marketTime = new Date(now.toLocaleString("en-US", { timeZone: marketHours.timezone }));
        const [openHour, openMinute] = marketHours.regularOpenTime.split(':').map(Number);
     ...
     */
    static getCrossMarketAdjustment(primaryMarket, tradingMarket) {
...
exports.MarketHoursManager = MarketHoursManager;
// IMF Vaulting System Engine
class ImfVaultingEngine {
    /**
     * Calculate scarcity multiplier based on supply and demand
     */
    static calculateScarcityMultiplier(vaultSettings) {
     ...
     */
    static shouldTriggerVaulting(vaultSettings, currentPrice, marketCap, volumeRatio) {
     ...
     */
    static calculateVaultingFee(vaultSettings, sharesAmount, currentPrice) {
        const feeRate = Number(vaultSettings.vaultingFee);
        const tradeValue = sharesAmount * currentPrice;
        return tradeValue * feeRate;
    }
}
exports.ImfVaultingEngine = ImfVaultingEngine;
// NPC Trading AI Engine
class NpcTradingEngine {
    /**
     * Calculate NPC trading decision based on personality and market conditions
     */
    static calculateTradingDecision(trader, assetPrice, priceHistory, marketSentiment, volumeProfile) {
        const personality = trader.tradingPersonality;
        ...
        switch (trader.traderType) {
            ...
            case 'momentum':
                ...
                if (volumeProfile > 1.5)
                    buyProbability += 0.2; // High volume confirmation
                break;
            case 'contrarian':
                // Contrarians trade against sentiment
                buyProbability -= marketSentiment * 0.3;
                if (momentum < -0.05)
                    buyProbability += 0.4; // Buy on dips
                break;
            case 'arbitrage':
     ...
     */
    static calculatePricingEfficiency(currentPrice, priceHistory) {
     ...
     */
    static updateTraderPerformance(trader, tradeResult) {
     ...
     */
    static async executeNpcTradingCycle(storage, marketEngine) {
            ...
            for (const trader of npcTraders) {
                try {
                    ...
                    for (const assetId of assetsToConsider) {
                        ...
                        const decision = this.calculateTradingDecision(trader, currentPrice, priceHistory, marketSentiment, volumeProfile);
...
exports.NpcTradingEngine = NpcTradingEngine;
// Information Tier Access Manager
class InformationTierManager {
    /**
     * Check if user has access to news based on their tier
     */
    static hasNewsAccess(userTier, article) {
        const now = new Date();
        switch (userTier) {
            case 'elite':
     ...
     */
    static getAnalysisQuality(userTier) {
        switch (userTier) {
            case 'elite':
                return {
                    accuracy: 0.95,
...
exports.InformationTierManager = InformationTierManager;
// Trading Firm Specialty Engine
class TradingFirmEngine {
    /**
     * Calculate trading bonus/penalty for a firm based on asset type
     */
    static calculateFirmBonus(firm, assetType, tradeSize) {
        const specialties = firm.primarySpecialties || [];
        const weaknesses = firm.weaknesses || [];
        const specialtyBonuses = firm.specialtyBonuses || {};
        const weaknessPenalties = firm.weaknessPenalties || {};
     ...
     */
    static calculateReputationImpact(firm, tradePerformance) {
...
exports.TradingFirmEngine = TradingFirmEngine;
