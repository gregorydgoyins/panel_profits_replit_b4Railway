...
const marvelCoverBot_1 = require("../services/marvelCoverBot");
const router = (0, express_1.Router)();
const coverBot = new marvelCoverBot_1.MarvelCoverBot();
/**
...
router.get('/status', async (req, res) => {
    try {
        ...
        const result = await marvelExpansionService_1.marvelExpansionService.fetchCharacters(1, 0);
        res.json({
            success: true,
            connected: true,
...
router.get('/counts', async (req, res) => {
    try {
        const [characters, comics, creators] = await Promise.all([
            marvelExpansionService_1.marvelExpansionService.fetchCharacters(1, 0),
            marvelExpansionService_1.marvelExpansionService.fetchComics(1, 0),
            marvelExpansionService_1.marvelExpansionService.fetchCreators(1, 0)
        ]);
        res.json({
            success: true,
            counts: {
...
router.post('/expand/characters', async (req, res) => {
    try {
        ...
        // Start expansion
        const progress = await marvelExpansionService_1.marvelExpansionService.expandCharacters(startOffset, maxToProcess);
        res.json({
            success: true,
            progress
        });
...
router.post('/expand/comics', async (req, res) => {
    try {
        ...
        console.log(`   Max To Process: ${maxToProcess}`);
        // Start expansion
        const progress = await marvelExpansionService_1.marvelExpansionService.expandComics(startOffset, maxToProcess);
        res.json({
            success: true,
            progress
        });
...
router.post('/expand/creators', async (req, res) => {
    try {
        ...
        console.log(`   Max To Process: ${maxToProcess}`);
        // Start expansion
        const progress = await marvelExpansionService_1.marvelExpansionService.expandCreators(startOffset, maxToProcess);
        res.json({
            success: true,
            progress
        });
...
router.get('/test/sample', async (req, res) => {
    try {
        ...
        if (resourceType === 'characters') {
            results = await marvelExpansionService_1.marvelExpansionService.fetchCharacters(limit, 0);
        }
        else if (resourceType === 'comics') {
            results = await marvelExpansionService_1.marvelExpansionService.fetchComics(limit, 0);
        }
        else if (resourceType === 'creators') {
            results = await marvelExpansionService_1.marvelExpansionService.fetchCreators(limit, 0);
        }
        else {
        ...
        res.json({
            ...
            sample: results.data.results.slice(0, 3).map((item) => ({
                id: item.id,
