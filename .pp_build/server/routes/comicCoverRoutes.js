...
exports.registerComicCoverRoutes = registerComicCoverRoutes;
const storage_1 = require("../storage");
const comicDataService_1 = require("../services/comicDataService");
function registerComicCoverRoutes(app) {
    ...
    app.get("/api/comic-covers/comic/:id", async (req, res) => {
        try {
            ...
            const comic = await comicDataService_1.comicDataService.fetchMarvelComicById(comicIdNum);
            if (!comic) {
    ...
    app.get("/api/comic-covers/key-issues", async (req, res) => {
        try {
            ...
            const comics = await comicDataService_1.comicDataService.fetchKeyIssues(limit);
            res.json({
                success: true,
                data: comics,
                count: comics.length
    ...
    app.get("/api/comic-covers/random", async (req, res) => {
        try {
            ...
            const comics = await comicDataService_1.comicDataService.fetchRandomComicCovers(limit);
            res.json({
                success: true,
                data: comics,
                count: comics.length
    ...
    app.get("/api/comic-covers/comic-of-the-day", async (req, res) => {
        try {
            const comic = await comicDataService_1.comicDataService.getComicOfTheDay();
            if (!comic) {
                return res.status(404).json({
                    success: false,
    ...
    app.get("/api/comic-covers/featured", async (req, res) => {
        try {
            ...
            const featured = await storage_1.storage.getFeaturedComics({
                featureType,
                isActive: true,
                limit
            });
    ...
    app.get("/api/comic-covers/hero-banner", async (req, res) => {
        try {
            ...
            const heroComics = await storage_1.storage.getFeaturedComics({
            ...
            if (comics.length === 0) {
                comics = await storage_1.storage.getFeaturedComics({
                    isActive: true,
                    limit
                });
            }
    ...
    app.get("/api/comic-covers/trending", async (req, res) => {
        try {
            const limit = req.query.limit ? parseInt(req.query.limit) : 12;
            const trendingComics = await storage_1.storage.getFeaturedComics({
    ...
    app.post("/api/comic-covers/import", async (req, res) => {
        try {
            const { ComicDataImportService } = await Promise.resolve().then(() => __importStar(require('../services/comicDataImportService.js')));
...
exports.default = registerComicCoverRoutes;
