...
const marvelDcDataImportService_js_1 = require("../services/marvelDcDataImportService.js");
const comicDataImportService_js_1 = require("../services/comicDataImportService.js");
...
router.post('/marvel-dc', async (req, res) => {
    try {
        ...
        const result = await marvelDcDataImportService_js_1.marvelDcDataImportService.importMarvelDcDataset();
        if (result.success) {
            res.status(200).json({
                success: true,
...
router.get('/marvel-dc/status', async (req, res) => {
    try {
        ...
        const assets = await storage_js_1.storage.getAssets({ type: 'media' });
        const marvelAssets = await storage_js_1.storage.getAssets();
        // Filter for Marvel/DC assets (basic heuristic)
        const marvelDcAssets = marvelAssets.filter(asset => asset.metadata && (asset.metadata.publisher === 'Marvel' ||
            asset.metadata.publisher === 'DC' ||
            ...
            data: {
                ...
                transformed: {
                    ...
                    metadata: {
                        ...
                        genres: testAsset.Genre.split(',').map((g) => g.trim())
                    }
                }
            }
        });
...
router.post('/marvel-comics', async (req, res) => {
    try {
        ...
        const result = await comicDataImportService_js_1.ComicDataImportService.importMarvelComicsCSV();
...
router.get('/marvel-comics/status', async (req, res) => {
    try {
        const statistics = await comicDataImportService_js_1.ComicDataImportService.getImportStatistics();
        res.json({
            success: true,
            data: {
                statistics,
