...
const pineconeAssetSeeder_1 = require("../services/pineconeAssetSeeder");
const pineconeMarketDataMigration_1 = require("../services/pineconeMarketDataMigration");
const assetInsertionService_1 = require("../services/assetInsertionService");
const kaggleAssetExpansion_1 = require("../services/kaggleAssetExpansion");
...
router.post("/search", async (req, res) => {
    try {
        ...
        // Query Pinecone
        const results = await pineconeService_1.pineconeService.querySimilar(embedding, topK);
        res.json({
            query,
...
router.get("/sample", async (req, res) => {
    try {
        ...
        const results = await pineconeService_1.pineconeService.querySimilar(zeroVector, 10);
        res.json({
            message: "Sample records from Pinecone index (no OpenAI required)",
            totalRecords: 63934,
            dimension: 1024,
            samples: results?.map((match) => ({
                id: match.id,
                score: match.score,
                metadata: match.metadata
            })) || []
...
router.post("/fetch", async (req, res) => {
    try {
        ...
        const records = await pineconeService_1.pineconeService.fetchVectors(ids);
        res.json({
...
router.post("/expand", async (req, res) => {
    try {
        ...
        const result = await pineconeAssetExpansion_1.pineconeAssetExpansion.expandAssetDatabase(samplesPerCategory);
        res.json(result);
    }
    catch (error) {
...
router.post("/seed-assets", async (req, res) => {
    try {
        const { batchSize = 100, totalSamples, samplesPerCategory = 100 } = req.body;
        // Validate parameters
        ...
        const result = await pineconeAssetSeeder_1.pineconeAssetSeeder.seedAssets({
            batchSize,
...
router.post("/seed-all-vectors", async (req, res) => {
    try {
        ...
        const expansionResult = await pineconeAssetExpansion_1.pineconeAssetExpansion.expandAllVectorsExhaustively();
...
router.post("/migrate-market-data", async (req, res) => {
    try {
        ...
        const result = await pineconeMarketDataMigration_1.pineconeMarketDataMigration.migrateMarketData();
        res.json(result);
    }
    catch (error) {
...
router.post("/kaggle/expand-all", async (req, res) => {
    try {
        ...
        console.log('═══════════════════════════════════════');
        const startTime = Date.now();
        const result = await kaggleAssetExpansion_1.kaggleAssetExpansion.expandAllKaggleAssets();
        const processingTime = Date.now() - startTime;
