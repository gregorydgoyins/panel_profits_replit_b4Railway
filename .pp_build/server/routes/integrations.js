...
const externalIntegrationsService_js_1 = require("../services/externalIntegrationsService.js");
const webhookManager_js_1 = require("../services/webhookManager.js");
/**
...
router.get('/', async (req, res) => {
    try {
        ...
        const divineConnections = integrations.map(integration => ({
            ...integration,
            mysticalName: externalIntegrationsService_js_1.INTEGRATION_CONFIGS[integration.integrationName]?.description || integration.integrationDisplayName,
            divineStatus: integration.status === 'connected' ? 'Aligned' : 'Seeking Harmony',
            cosmicPower: externalIntegrationsService_js_1.INTEGRATION_CONFIGS[integration.integrationName]?.mysticalPower || 5,
        }));
        res.json({
            divineConnections,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const integration = await storage_js_1.storage.getExternalIntegration(req.params.id);
        if (!integration) {
        ...
        const analytics = await storage_js_1.storage.getUserIntegrationAnalytics(userId, {
            integrationName: integration.integrationName,
        });
        res.json({
            integration: {
                ...integration,
                mysticalName: externalIntegrationsService_js_1.INTEGRATION_CONFIGS[integration.integrationName]?.description,
                cosmicPower: externalIntegrationsService_js_1.INTEGRATION_CONFIGS[integration.integrationName]?.mysticalPower,
...
router.post('/', async (req, res) => {
    try {
        ...
        const validationSchema = schema_js_1.insertExternalIntegrationSchema.extend({
            credentials: zod_1.z.object({}).passthrough(), // Raw credentials to be encrypted
        });
        const { credentials, ...integrationData } = validationSchema.parse(req.body);
        // Check if integration type is supported
        if (!externalIntegrationsService_js_1.INTEGRATION_CONFIGS[integrationData.integrationName]) {
            return res.status(400).json({ error: 'Unsupported divine realm' });
        }
        // Create integration with encrypted credentials
        const integration = await externalIntegrationsService_js_1.ExternalIntegrationsService.createIntegration(userId, integrationData.integrationName, credentials, integrationData.configuration || {});
        res.status(201).json({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updatedIntegration = await storage_js_1.storage.updateExternalIntegration(req.params.id, updates);
        res.json({
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await storage_js_1.storage.deleteExternalIntegration(req.params.id);
...
router.post('/:id/test', async (req, res) => {
    try {
        ...
        const result = await externalIntegrationsService_js_1.ExternalIntegrationsService.testConnection(req.params.id);
        res.json({
...
router.post('/:id/sync', async (req, res) => {
    try {
        ...
        const syncLog = await externalIntegrationsService_js_1.ExternalIntegrationsService.synchronizeData(req.params.id, syncType, direction, dataType);
        res.json({
...
router.get('/:id/webhooks', async (req, res) => {
    try {
        ...
        const webhooks = await storage_js_1.storage.getIntegrationWebhooks(req.params.id);
        res.json({
...
router.post('/:id/webhooks', async (req, res) => {
    try {
        ...
        const webhookData = schema_js_1.insertIntegrationWebhookSchema.parse({
            ...req.body,
            integrationId: req.params.id,
        });
        const webhook = await webhookManager_js_1.WebhookManager.createWebhook(webhookData);
        res.status(201).json({
...
router.post('/:id/webhooks/:webhookId/test', async (req, res) => {
    try {
        ...
        const result = await webhookManager_js_1.WebhookManager.testWebhook(req.params.webhookId);
        res.json({
            success: result.success,
            message: result.success
...
router.get('/workflows', async (req, res) => {
    try {
        ...
        const workflows = await storage_js_1.storage.getUserWorkflowAutomations(userId);
        res.json({
...
router.post('/workflows', async (req, res) => {
    try {
        ...
        const workflowData = schema_js_1.insertWorkflowAutomationSchema.parse({
            ...req.body,
            userId,
        });
        const workflow = await storage_js_1.storage.createWorkflowAutomation(workflowData);
        res.status(201).json({
...
router.post('/workflows/:id/execute', async (req, res) => {
    try {
        ...
        const workflow = await storage_js_1.storage.getWorkflowAutomation(req.params.id);
        if (!workflow || workflow.userId !== userId) {
            return res.status(404).json({ error: 'Sacred ritual not found' });
        }
        // Create execution record
        const execution = await storage_js_1.storage.createWorkflowExecution({
    ...
    try {
        ...
        res.json({
            ...
            connections: integrations.map(i => ({
                ...
                mysticalName: externalIntegrationsService_js_1.INTEGRATION_CONFIGS[i.integrationName]?.description,
            })),
        });
    }
    catch (error) {
...
router.post('/webhooks/incoming/:integrationName', async (req, res) => {
    try {
        ...
        const result = await webhookManager_js_1.WebhookManager.processIncoming(integrationName, headers, body, rawBody);
        if (result.success) {
