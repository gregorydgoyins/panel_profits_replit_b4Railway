...
const enhancedAiMarketIntelligence_js_1 = require("../services/enhancedAiMarketIntelligence.js");
const storage_js_1 = require("../storage.js");
const router = express_1.default.Router();
// Request validation schemas
const generateInsightsSchema = zod_1.z.object({
    assetIds: zod_1.z.array(zod_1.z.string()).optional(),
    analysisType: zod_1.z.enum(['full', 'quick', 'targeted']).default('full'),
    includeHouseSpecific: zod_1.z.boolean().default(true),
    includeKarmaAlignment: zod_1.z.boolean().default(true)
});
const battlePredictionSchema = zod_1.z.object({
    character1Id: zod_1.z.string(),
    character2Id: zod_1.z.string(),
    battleType: zod_1.z.enum(['power_clash', 'strategy_battle', 'moral_conflict', 'crossover_event']).default('power_clash')
});
const marketAnomalySchema = zod_1.z.object({
    assetIds: zod_1.z.array(zod_1.z.string()).optional(),
    severityThreshold: zod_1.z.enum(['low', 'medium', 'high', 'critical']).default('medium')
});
/**
...
router.get('/enhanced-predictions', async (req, res) => {
    try {
        ...
        const parsedAssetIds = assetIds ? (Array.isArray(assetIds) ? assetIds : [assetIds]) : undefined;
        // Get assets to analyze
        let assets = [];
        if (parsedAssetIds && parsedAssetIds.length > 0) {
            // Get specific assets
            assets = await Promise.all(parsedAssetIds.map(id => storage_js_1.storage.getAsset(id)).filter(Boolean));
        }
        else {
            // Get trending or popular assets
            assets = await storage_js_1.storage.getAssets({ type: 'character' });
        ...
        const enhancedAssets = assets.map(asset => ({
            ...
            historicalPrices: asset.metadata?.historicalPrices || Array.from({ length: 30 }, (_, i) => parseFloat(asset.metadata?.currentPrice || '100') * (0.9 + Math.random() * 0.2)),
        ...
        const predictions = await enhancedAiMarketIntelligence_js_1.enhancedAiMarketIntelligence.generateEnhancedPredictions(enhancedAssets, user);
...
router.post('/enhanced-predictions', async (req, res) => {
    try {
        const user = req.user;
        const { assetIds, analysisType, includeHouseSpecific, includeKarmaAlignment } = generateInsightsSchema.parse(req.body);
        console.log('🔮 Enhanced AI Oracle: Generating predictions for', user?.email || 'anonymous user');
        // Get assets to analyze
        let assets = [];
        if (assetIds && assetIds.length > 0) {
            // Get specific assets
            assets = await Promise.all(assetIds.map(id => storage_js_1.storage.getAsset(id)).filter(Boolean));
        }
        else {
            // Get trending or popular assets
            assets = await storage_js_1.storage.getAssets({ type: 'character' });
...
router.get('/battle-predictions', async (req, res) => {
    try {
        ...
        const characters = await storage_js_1.storage.getAssets({ type: 'character' });
        ...
        for (let i = 0; i < Math.min(popularCharacters.length - 1, 3); i++) {
            ...
            if (char1 && char2) {
                ...
                const enhancedChar1 = {
                    ...
                    historicalPrices: Array.from({ length: 30 }, (_, i) => parseFloat(char1.metadata?.currentPrice || '100') * (0.9 + Math.random() * 0.2)),
                ...
                const enhancedChar2 = {
                    ...
                    historicalPrices: Array.from({ length: 30 }, (_, i) => parseFloat(char2.metadata?.currentPrice || '100') * (0.9 + Math.random() * 0.2)),
                    volume24h: 1000 + Math.random() * 5000,
                    marketCap: 1000000 + Math.random() * 10000000,
                    volatility: 0.1 + Math.random() * 0.3,
                ...
                const battlePrediction = await enhancedAiMarketIntelligence_js_1.enhancedAiMarketIntelligence.predictCharacterBattle(enhancedChar1, enhancedChar2, battleType, user);
...
router.post('/battle-predictions', async (req, res) => {
    try {
        ...
        const character1 = await storage_js_1.storage.getAsset(character1Id);
        const character2 = await storage_js_1.storage.getAsset(character2Id);
        ...
        const enhancedChar1 = {
            ...
            historicalPrices: Array.from({ length: 30 }, (_, i) => parseFloat(character1.metadata?.currentPrice || '100') * (0.9 + Math.random() * 0.2)),
            volume24h: 1000 + Math.random() * 5000,
            marketCap: 1000000 + Math.random() * 10000000,
            volatility: 0.1 + Math.random() * 0.3,
        ...
        const enhancedChar2 = {
            ...
            historicalPrices: Array.from({ length: 30 }, (_, i) => parseFloat(character2.metadata?.currentPrice || '100') * (0.9 + Math.random() * 0.2)),
            volume24h: 1000 + Math.random() * 5000,
            marketCap: 1000000 + Math.random() * 10000000,
            volatility: 0.1 + Math.random() * 0.3,
...
router.get('/market-anomalies', async (req, res) => {
    try {
        ...
        if (assetIds && assetIds.length > 0) {
            ...
            assets = await storage_js_1.storage.getAssets();
        ...
        // Detect anomalies
        const anomalies = await enhancedAiMarketIntelligence_js_1.enhancedAiMarketIntelligence.detectMarketAnomalies(enhancedAssets);
...
router.get('/advanced-insights', async (req, res) => {
    try {
        ...
        const characters = await storage_js_1.storage.getAssets({ type: 'character' });
        const comics = await storage_js_1.storage.getAssets({ type: 'comic' });
        const creators = await storage_js_1.storage.getAssets({ type: 'creator' });
        const assets = [
        ...
        const insights = await enhancedAiMarketIntelligence_js_1.enhancedAiMarketIntelligence.generateAdvancedMarketInsights(enhancedAssets, user);
