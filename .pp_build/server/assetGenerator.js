...
exports.assetGenerator = exports.AssetGenerator = void 0;
const crypto_1 = __importDefault(require("crypto"));
const tickerGenerator_js_1 = require("./tickerGenerator.js");
 ...
 */
function determineCategory(char, source) {
 ...
 */
function extractSeriesAndYear(char) {
    let series;
    let year;
...
class AssetGenerator {
    constructor() {
        this.COMIC_VINE_API_KEY = process.env.COMIC_VINE_API_KEY;
        this.MARVEL_PUBLIC_KEY = process.env.MARVEL_API_PUBLIC_KEY;
        this.MARVEL_PRIVATE_KEY = process.env.MARVEL_API_PRIVATE_KEY;
        this.SUPERHERO_API_TOKEN = process.env.SUPERHERO_API_TOKEN;
    }
    /**
     * Calculate share float based on market cap to keep price in desired range
     * Target: Most assets should be $1-$500/share
     */
    calculateFloat(marketCap, targetPriceRange = [50, 200]) {
        ...
        if (float < 100000)
            return Math.ceil(float / 10000) * 10000;
        if (float < 1000000)
            return Math.ceil(float / 100000) * 100000;
        return Math.ceil(float / 1000000) * 1000000;
    }
    /**
     * Calculate initial price based on float
     */
    calculateInitialPrice(marketCap, float) {
        return Number((marketCap / float).toFixed(2));
    }
    /**
     * Fetch characters from Comic Vine
     */
    async fetchComicVineCharacters(limit = 100, offset = 0) {
        if (!this.COMIC_VINE_API_KEY) {
            console.warn('Comic Vine API key not found');
            return [];
        }
        try {
            const response = await fetch(`https://comicvine.gamespot.com/api/characters/?api_key=${this.COMIC_VINE_API_KEY}&format=json&limit=${limit}&offset=${offset}&sort=name:asc`, {
                headers: {
     ...
     */
    async fetchMarvelCharacters(limit = 100) {
        ...
        try {
            ...
            const hash = crypto_1.default
                ...
                .digest('hex');
            const response = await fetch(`https://gateway.marvel.com/v1/public/characters?ts=${ts}&apikey=${this.MARVEL_PUBLIC_KEY}&hash=${hash}&limit=${limit}&orderBy=name`);
            if (!response.ok) {
     ...
     */
    async fetchSuperheroCharacters(ids) {
        ...
        for (const id of ids) {
            try {
                const response = await fetch(`https://superheroapi.com/api/${this.SUPERHERO_API_TOKEN}/${id}`);
                if (!response.ok)
                    continue;
     ...
     */
    async fetchComicVineTeams(limit = 100, offset = 0) {
        if (!this.COMIC_VINE_API_KEY) {
            console.warn('Comic Vine API key not found');
            return [];
        }
        try {
            const response = await fetch(`https://comicvine.gamespot.com/api/teams/?api_key=${this.COMIC_VINE_API_KEY}&format=json&limit=${limit}&offset=${offset}&sort=name:asc`, {
                headers: {
                    'User-Agent': 'Panel Profits Trading Platform'
                }
            });
     ...
     */
    async fetchComicVineLocations(limit = 100, offset = 0) {
        if (!this.COMIC_VINE_API_KEY) {
            console.warn('Comic Vine API key not found');
            return [];
        }
        try {
            const response = await fetch(`https://comicvine.gamespot.com/api/locations/?api_key=${this.COMIC_VINE_API_KEY}&format=json&limit=${limit}&offset=${offset}&sort=name:asc`, {
                headers: {
                    'User-Agent': 'Panel Profits Trading Platform'
                }
            });
     ...
     */
    async fetchComicVineObjects(limit = 100, offset = 0) {
        if (!this.COMIC_VINE_API_KEY) {
            console.warn('Comic Vine API key not found');
            return [];
        }
        try {
            const response = await fetch(`https://comicvine.gamespot.com/api/objects/?api_key=${this.COMIC_VINE_API_KEY}&format=json&limit=${limit}&offset=${offset}&sort=name:asc`, {
                headers: {
                    'User-Agent': 'Panel Profits Trading Platform'
                }
            });
     ...
     */
    async fetchComicVineCreators(limit = 100) {
        if (!this.COMIC_VINE_API_KEY) {
            console.warn('Comic Vine API key not found');
            return [];
        }
        try {
            const response = await fetch(`https://comicvine.gamespot.com/api/people/?api_key=${this.COMIC_VINE_API_KEY}&format=json&limit=${limit}&sort=name:asc`, {
                headers: {
                    'User-Agent': 'Panel Profits Trading Platform'
                }
            });
     ...
     */
    estimateCharacterMarketCap(char) {
     ...
     */
    estimateMarvelCharacterMarketCap(char) {
     ...
     */
    estimateSuperheroMarketCap(char) {
     ...
     */
    async generateAssets(assetData) {
        let inserted = 0;
        for (const data of assetData) {
            try {
                // Generate hierarchical ticker: SERIES.YEAR.CATEGORY.INDEX
                const ticker = tickerGenerator_js_1.tickerGenerator.generateHierarchicalTicker({
                ...
                const validated = schema_js_1.insertAssetSchema.parse(asset);
    ...
    async generateAssetUniverse(options = {}) {
        const { comicVineCharacters = 100, comicVineTeams = 50, comicVineLocations = 50, comicVineObjects = 50, comicVineCreators = 50, marvelCharacters = 100, superheroIds = [] } = options;
        ...
        console.log(`🎯 Ticker space used: ${tickerGenerator_js_1.tickerGenerator.getStats().totalUsed.toLocaleString()}`);
...
exports.assetGenerator = new AssetGenerator();
