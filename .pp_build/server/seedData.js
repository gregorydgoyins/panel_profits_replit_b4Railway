...
exports.SEED_MARKET_EVENTS = exports.SEED_COMIC_ASSETS = void 0;
exports.seedMarketData = seedMarketData;
...
async function seedMarketData() {
    ...
    try {
        // Create assets
        const createdAssets = [];
        for (const assetData of exports.SEED_COMIC_ASSETS) {
            const existingAsset = await storage_js_1.storage.getAssetBySymbol(assetData.symbol);
            if (!existingAsset) {
                const asset = await storage_js_1.storage.createAsset(assetData);
        ...
        for (const asset of createdAssets) {
            const existingPrice = await storage_js_1.storage.getAssetCurrentPrice(asset.id);
            if (!existingPrice) {
                ...
                await storage_js_1.storage.createAssetCurrentPrice(priceData);
        ...
        const marvelAssets = createdAssets.filter(a => a.metadata?.publisher === 'Marvel').map(a => a.id);
        const dcAssets = createdAssets.filter(a => a.metadata?.publisher === 'DC').map(a => a.id);
        const spiderManAsset = createdAssets.find(a => a.symbol === 'SPIDER')?.id;
        // Update event affected assets
        exports.SEED_MARKET_EVENTS[0].affectedAssets = spiderManAsset ? [spiderManAsset] : []; // Spider-Man movie
        exports.SEED_MARKET_EVENTS[1].affectedAssets = assetIds; // Comic-Con affects all
        exports.SEED_MARKET_EVENTS[2].affectedAssets = marvelAssets; // Marvel announcement
        exports.SEED_MARKET_EVENTS[3].affectedAssets = dcAssets; // DC concerns
        for (const eventData of exports.SEED_MARKET_EVENTS) {
            const event = await storage_js_1.storage.createMarketEvent(eventData);
            console.log(`📰 Created market event: ${event.title}`);
        }
        console.log('✅ Market data seeding completed successfully!');
        console.log(`📊 Created ${createdAssets.length} assets and ${exports.SEED_MARKET_EVENTS.length} market events`);
    }
    catch (error) {
...
async function generateHistoricalData() {
    ...
    try {
        const assets = await storage_js_1.storage.getAssets({ limit: 100 });
        const timeframes = ['1m', '5m', '15m', '1h', '4h', '1d'];
        for (const asset of assets.slice(0, 5)) { // Only for seed assets
            console.log(`Generating history for ${asset.symbol}...`);
            const currentPrice = await storage_js_1.storage.getAssetCurrentPrice(asset.id);
            if (!currentPrice)
                continue;
            ...
            for (let i = 30; i >= 1; i--) {
                ...
                await storage_js_1.storage.createMarketData(marketData);
            }
        }
