...
const weightedMarketCapService_1 = require("./services/weightedMarketCapService");
/**
...
async function testWeightedPricing() {
    ...
    for (const comic of testComics) {
        console.log(`\n📖 Testing: ${comic}`);
        console.log('-'.repeat(70));
        const result = await weightedMarketCapService_1.weightedMarketCapService.calculateWeightedPricing(comic, 100);
        if (result) {
            console.log(`\n✅ Results:`);
