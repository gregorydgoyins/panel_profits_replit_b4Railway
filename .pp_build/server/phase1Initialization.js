...
exports.Phase1Initializer = void 0;
const databaseStorage_js_1 = require("./databaseStorage.js");
const phase1SeedData_js_1 = require("./phase1SeedData.js");
class Phase1Initializer {
     ...
     */
    static async initializeTradingFirms() {
        console.log('Initializing Seven House Trading Firms...');
        for (const firmData of phase1SeedData_js_1.sevenHouseTradingFirms) {
            try {
                // Check if firm already exists
                const existing = await databaseStorage_js_1.databaseStorage.getTradingFirmByCode(firmData.firmCode);
                if (!existing) {
                    await databaseStorage_js_1.databaseStorage.createTradingFirm(firmData);
     ...
     * Initialize Global Market Hours
     */
    static async initializeGlobalMarketHours() {
        console.log('Initializing Global Market Hours...');
        for (const marketData of phase1SeedData_js_1.globalMarketHours) {
            try {
                // Check if market already exists
                const existing = await databaseStorage_js_1.databaseStorage.getGlobalMarketHours(marketData.marketCode);
                if (!existing) {
                    await databaseStorage_js_1.databaseStorage.createGlobalMarketHours(marketData);
     ...
     * Initialize Information Tiers
     */
    static async initializeInformationTiers() {
        console.log('Initializing Information Tiers...');
        for (const tierData of phase1SeedData_js_1.informationTiers) {
            try {
                // Check if tier already exists
                const existing = await databaseStorage_js_1.databaseStorage.getInformationTier(tierData.tierName);
                if (!existing) {
                    await databaseStorage_js_1.databaseStorage.createInformationTier(tierData);
     ...
     */
    static async initializeSampleAssetMappings() {
        console.log('Initializing Sample Asset Financial Mappings...');
        for (const mappingData of phase1SeedData_js_1.sampleAssetMappings) {
            try {
                // Check if mapping already exists
                const existing = await databaseStorage_js_1.databaseStorage.getAssetFinancialMapping(mappingData.assetId);
                if (!existing) {
                    await databaseStorage_js_1.databaseStorage.createAssetFinancialMapping(mappingData);
     ...
     * Initialize Sample News Articles
     */
    static async initializeSampleNews() {
        console.log('Initializing Sample News Articles...');
        for (const articleData of phase1SeedData_js_1.sampleNewsArticles) {
            try {
                await databaseStorage_js_1.databaseStorage.createNewsArticle(articleData);
     ...
     * Initialize Sample NPC Traders
     */
    static async initializeSampleNpcTraders() {
        console.log('Initializing Sample NPC Traders...');
        for (const traderData of phase1SeedData_js_1.sampleNpcTraders) {
            try {
                await databaseStorage_js_1.databaseStorage.createNpcTrader(traderData);
     ...
     */
    static async initializeAssetPhase1(assetId, assetName, assetType) {
        ...
        try {
            ...
            await databaseStorage_js_1.databaseStorage.createImfVaultSettings(vaultSettings);
            console.log(`  ✓ Created IMF vault settings for ${assetName}`);
            // Create Asset Financial Mapping
            let instrumentType;
            let instrumentSubtype;
            let securityType;
            switch (assetType) {
                case 'hero':
            ...
            await databaseStorage_js_1.databaseStorage.createAssetFinancialMapping(financialMapping);
     ...
     */
    static async isPhase1Initialized() {
        try {
    ...
    static async getInitializationStatus() {
        try {
            const [firms, markets, tiers, npcTraders] = await Promise.all([
                databaseStorage_js_1.databaseStorage.getAllTradingFirms(),
                databaseStorage_js_1.databaseStorage.getAllGlobalMarketHours(),
                databaseStorage_js_1.databaseStorage.getAllInformationTiers(),
                databaseStorage_js_1.databaseStorage.getActiveNpcTraders()
            ]);
            // Count asset mappings by type
            const [stockMappings, bondMappings, preferredMappings, etfMappings] = await Promise.all([
                databaseStorage_js_1.databaseStorage.getAssetFinancialMappingsByType('common_stock'),
                databaseStorage_js_1.databaseStorage.getAssetFinancialMappingsByType('bond'),
                databaseStorage_js_1.databaseStorage.getAssetFinancialMappingsByType('preferred_stock'),
                databaseStorage_js_1.databaseStorage.getAssetFinancialMappingsByType('etf')
            ]);
            ...
            const recentNews = await databaseStorage_js_1.databaseStorage.getNewsArticlesByTier('free', 100);
            const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            const newsArticles = recentNews.filter(article => new Date(article.createdAt) >= thirtyDaysAgo).length;
            return {
...
exports.Phase1Initializer = Phase1Initializer;
