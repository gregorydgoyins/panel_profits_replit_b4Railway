...
exports.phase1Services = exports.Phase1ScheduledServices = exports.DEFAULT_SERVICE_CONFIG = void 0;
const storage_js_1 = require("./storage.js");
const tradingEngine_js_1 = require("./tradingEngine.js");
const orderMatchingEngine_js_1 = require("./services/orderMatchingEngine.js");
...
class Phase1ScheduledServices {
    constructor(config = exports.DEFAULT_SERVICE_CONFIG) {
     ...
     */
    async updateOptionsChains() {
        try {
            // Get all options chains that need updates
            const optionsChains = await storage_js_1.storage.getAllOptionsChains();
            ...
            for (const chain of optionsChains) {
                try {
                    ...
                    const currentPrice = await storage_js_1.storage.getAssetCurrentPrice(chain.underlyingAssetId);
                    if (!currentPrice)
                        continue;
                    ...
                    if (timeToExpiration <= 0) {
                        ...
                        await storage_js_1.storage.updateOptionsChain(chain.id, {
                            isActive: false,
                            lastUpdate: new Date()
                        });
                        continue;
                    ...
                    const pricing = tradingEngine_js_1.OptionsCalculator.calculateOptionPricing(underlyingPrice, strikePrice, timeToExpiration, 0.05, // risk-free rate
                    volatility, chain.optionType);
     ...
     */
    startNpcTradingAutomation() {
        const interval = setInterval(async () => {
            try {
                const results = await tradingEngine_js_1.NpcTradingEngine.executeNpcTradingCycle(storage_js_1.storage, null);
     ...
     */
    async performMarginMaintenance() {
        try {
            const marginAccounts = await storage_js_1.storage.getAllMarginAccounts();
            ...
            for (const account of marginAccounts) {
                try {
                    ...
                    if (marginEquity < maintenanceMargin && marginDebt > 0) {
                        ...
                        await storage_js_1.storage.updateMarginAccount(account.userId, account.portfolioId, {
     ...
     */
    async distributeInformationTierNews() {
        try {
            // Get all active news articles that haven't been distributed yet
            const pendingNews = await storage_js_1.storage.getNewsArticles({
                isActive: true,
            ...
            const informationTiers = await storage_js_1.storage.getAllInformationTiers();
            for (const newsArticle of pendingNews) {
                try {
                    ...
                    const eligibleUsers = await storage_js_1.storage.getUsersByInformationTier(requiredTier);
                    ...
                    await storage_js_1.storage.updateNewsArticle(newsArticle.id, {
...
exports.Phase1ScheduledServices = Phase1ScheduledServices;
// Global instance for use throughout the application
exports.phase1Services = new Phase1ScheduledServices();
