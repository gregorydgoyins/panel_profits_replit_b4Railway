...
import type { 
  TraderStats, LeaderboardCategory, UserAchievement, 
  User, Order, Portfolio, Holding 
...
export class LeaderboardService {
  ...
  async initializeDefaultCategories(): Promise<void> {
    const existingCategories = await databaseStorage.getLeaderboardCategories();
    ...
    if (existingCategories.length === 0) {
      ...
      for (const category of defaultCategories) {
        await databaseStorage.createLeaderboardCategory(category);
      }
    }
  }

  // Calculate trading performance from order and portfolio data
  async calculateTradingPerformance(userId: string, order: Order): Promise<TradingPerformance> {
    // Get user's portfolio to calculate current value
    const portfolio = await databaseStorage.getUserDefaultPortfolio(userId);
    if (!portfolio) {
      throw new Error('User portfolio not found');
    }

    // Get current holdings for portfolio value calculation
    const holdings = await databaseStorage.getPortfolioHoldings(portfolio.id);
    let portfolioValue = parseFloat(portfolio.cashBalance || "0");

    // Calculate current holdings value
    for (const holding of holdings) {
      const currentPrice = await databaseStorage.getAssetCurrentPrice(holding.assetId);
      if (currentPrice) {
  ...
  async updateTraderStatsFromOrder(userId: string, order: Order): Promise<TraderStats | undefined> {
    try {
      const performance = await this.calculateTradingPerformance(userId, order);
      
      const updatedStats = await databaseStorage.updateTraderStatsFromTrade(userId, {
  ...
  async generateLeaderboard(categoryId: string, limit: number = 50): Promise<LeaderboardData> {
    const category = await databaseStorage.getLeaderboardCategory(categoryId);
    if (!category) {
      throw new Error('Leaderboard category not found');
    }

    const rawEntries = await databaseStorage.getLeaderboardByCategoryId(categoryId, limit);
    ...
    const entries: LeaderboardEntry[] = rawEntries.map((entry, index) => ({
      ...
      userId: entry.userId,
      user: entry.user,
      stats: entry as TraderStats,
      change: 0 // TODO: Calculate rank change from previous period
    }));

    // Get total participants
    const allStats = await databaseStorage.getAllTraderStats({ minTrades: 1 });
  ...
  async getMultipleLeaderboards(categoryIds: string[], limit: number = 25): Promise<LeaderboardData[]> {
    const leaderboards = await Promise.all(
      categoryIds.map(id => this.generateLeaderboard(id, limit))
    );
    return leaderboards;
  }

  // Get user's ranking across all categories
  async getUserRankings(userId: string): Promise<Array<{
    category: LeaderboardCategory;
    rank: number;
    totalUsers: number;
    stats: TraderStats;
  }>> {
    const categories = await databaseStorage.getLeaderboardCategories({ isActive: true });
    
    const rankings = await Promise.all(
      categories.map(async (category) => {
        const ranking = await databaseStorage.getUserRankInCategory(
          userId, 
    ...
    return rankings.filter(r => r !== null) as Array<{
      category: LeaderboardCategory;
      rank: number;
      totalUsers: number;
      stats: TraderStats;
  ...
  async processAchievements(userId: string, context: 'trade_completed' | 'milestone_reached' | 'streak_achieved'): Promise<UserAchievement[]> {
    return await databaseStorage.checkAndAwardAchievements(userId, context);
  }
  ...
  async getLeaderboardOverview(): Promise<{
    ...
    const overview = await databaseStorage.getLeaderboardOverview();
    
    // Get recent achievements (last 24 hours)
    const allStats = await databaseStorage.getAllTraderStats({ limit: 10 });
    const recentAchievements: Array<UserAchievement & { user: User }> = [];
    
    for (const stats of allStats) { // Limit to prevent performance issues
      const user = await databaseStorage.getUser(stats.userId);
      if (!user) continue;
      const achievements = await databaseStorage.getUserAchievements(user.id, { isVisible: true });
  ...
  async calculateRankChanges(categoryId: string): Promise<Map<string, number>> {
  ...
  async recalculateAllRankings(): Promise<void> {
    await databaseStorage.recalculateAllTraderStats();
    await databaseStorage.updateLeaderboardRankings();
  }
  ...
  async getTradingActivitySummary(timeframe: 'daily' | 'weekly' | 'monthly'): Promise<{
    ...
    const summary = await databaseStorage.getTradingActivitySummary(timeframe);
    ...
    const allStats = await databaseStorage.getAllTraderStats({ limit: 20 });
    for (const stats of allStats) { // Limit for performance
      const user = await databaseStorage.getUser(stats.userId);
      if (!user) continue;
      const achievements = await databaseStorage.getUserAchievements(user.id);
      achievementCount += achievements.filter(a => 
        a.unlockedAt && new Date(a.unlockedAt) >= cutoffDate
      ).length;
    }
