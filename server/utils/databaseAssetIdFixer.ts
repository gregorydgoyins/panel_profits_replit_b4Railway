 ...
 */
export async function fixCharacterIdsAsAssetIds(): Promise<AssetIdMapping[]> {
  console.log('🔍 Checking for character IDs being used as asset IDs...');
  
  const mappings: AssetIdMapping[] = [];
 ...
 */
async function updateAssetIdReferences(mapping: AssetIdMapping): Promise<void> {
  ...
  try {
    ...
    await storage.updateAsset(mapping.oldId, { id: mapping.newId } as any);
    
    // 2. Update current prices
    const currentPrice = await storage.getAssetCurrentPrice(mapping.oldId);
    if (currentPrice) {
 ...
 */
export async function validateAssetIds(): Promise<boolean> {
 ...
 */
export async function cleanupAssetIds(): Promise<void> {
