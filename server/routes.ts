...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // CLZ Comics Integration - Test API connection with username
  app.get('/api/admin/clz/test', async (req: any, res) => {
    try {
      const { clzComicsService } = await import('./services/clzComicsService');
      
      // Get username from query param or use default test
      const username = req.query.username as string || 'demo';
      
      console.log(`📚 Testing CLZ Comics API with username: ${username}`);
      
      const collection = await clzComicsService.getUserCollection(username, 10);
      
      if (collection.totalCount === 0) {
        return res.json({
          success: false,
          message: `No comics found for CLZ user: ${username}. Make sure the collection is public.`,
          totalCount: 0,
          samples: []
        });
      }
      
      const samples = collection.comics.slice(0, 5).map(comic => ({
        title: comic.title,
        series: comic.series,
        issue: comic.issue,
        publisher: comic.publisher,
        value: comic.value ? `$${comic.value.toFixed(2)}` : 'N/A',
        coverUrl: comic.coverUrl || 'No cover',
        grade: comic.grade || 'Ungraded'
      }));
      
      res.json({
        success: true,
        message: `CLZ Comics API is working! Found ${collection.totalCount} comics for user: ${username}`,
        username,
        totalCount: collection.totalCount,
        samples
      });
    } catch (error) {
      console.error('❌ CLZ test failed:', error);
      res.status(500).json({ 
        success: false, 
        error: 'Failed to test CLZ Comics connection',
        details: (error as Error).message
      });
    }
  });

  // CLZ Comics - Search comics
  app.get('/api/clz/search', async (req: any, res) => {
    try {
      const { clzComicsService } = await import('./services/clzComicsService');
      ...
      const comics = await clzComicsService.searchComics(query, username);
      res.json({ success: true, comics });
    } catch (error) {
      console.error('❌ CLZ search failed:', error);
      res.status(500).json({ error: 'Failed to search CLZ Comics' });
    }
  });

  // CLZ Comics - Get covers for series
  app.get('/api/clz/covers/:series', async (req: any, res) => {
    try {
      const { clzComicsService } = await import('./services/clzComicsService');
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 20;
      
      const covers = await clzComicsService.getCoversForSeries(req.params.series, limit);
      res.json({ success: true, covers });
    } catch (error) {
      console.error('❌ Failed to get covers:', error);
      res.status(500).json({ error: 'Failed to get comic covers' });
    }
  });
