...
import {
  users, tradingConsequences, orders, assets, detailedKarmaActions,
  karmicProfiles, alignmentThresholds
} from '@shared/schema.js';
import type {
  User, Order, InsertTradingConsequence, TradingConsequence, Asset,
  InsertDetailedKarmaAction
} from '@shared/schema.js';
import { karmicAlignmentService } from './karmicAlignmentService.js';
 ...
 */
export class TradingConsequencesService {

  // Alignment-based trading modifiers
  private readonly ALIGNMENT_MODIFIERS = {
  ...
  private readonly HOUSE_TRADING_EFFECTS = {
    heroes: {
   ...
   */
  async applyTradingConsequences(
    userId: string,
    order: Order,
    asset: Asset,
    basePrice: number
    ...
    consequences: TradingConsequence[];
    karmaImpact: number;
    mysticalEvents: string[];
  }> {
    ...
    const consequences: InsertTradingConsequence[] = [];
    const mysticalEvents: string[] = [];
    ...
    if (currentUser.houseId) {
      const houseEffect = await this.applyHouseConsequences(
        userId, order, asset, modifiedPrice, currentUser.houseId
      );
    ...
    const savedConsequences: TradingConsequence[] = [];
    for (const consequence of consequences) {
      const [savedConsequence] = await db.insert(tradingConsequences)
        .values(consequence)
        .returning();
   ...
   */
  private async applyLawfulConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number
  ) {
   ...
   */
  private async applyChaoticConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number
  ) {
   ...
   */
  private async applyGoodConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number
  ) {
   ...
   */
  private async applyEvilConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number
  ) {
   ...
   */
  private async applyHouseConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number, houseId: string
  ) {
    let modifiedPrice = basePrice;
    const consequences: InsertTradingConsequence[] = [];
    const mysticalEvents: string[] = [];
    ...
    const houseEffects = this.HOUSE_TRADING_EFFECTS[houseId as keyof typeof this.HOUSE_TRADING_EFFECTS];
    if (!houseEffects) {
      return { modifiedPrice, consequences, mysticalEvents, karmaImpact };
    }
   ...
   */
  private async applyKarmaConsequences(
    userId: string, order: Order, asset: Asset, basePrice: number, karma: number
  ) {
    let modifiedPrice = basePrice;
    const consequences: InsertTradingConsequence[] = [];
    const mysticalEvents: string[] = [];
   ...
   */
  private async recordTradingKarmaAction(
    userId: string, order: Order, asset: Asset, karmaImpact: number, mysticalEvents: string[]
  ) {
    const actionType = this.determineKarmaActionType(order, karmaImpact);
    const description = `Trading ${asset.name} - ${mysticalEvents.join(' | ')}`;

    await karmicAlignmentService.recordKarmaAction(userId, actionType, {
      assetId: asset.id,
      orderId: order.id,
  ...
  // Helper methods
  private calculateMarketVolatility(asset: Asset): number {
    // Simplified volatility calculation
    return (Math.random() * 0.1) - 0.05; // -5% to +5%
  }

  private async calculateCommunityTradingSentiment(asset: Asset): Promise<number> {
    // Simplified community sentiment calculation
    return Math.random(); // 0 to 1
  }

  private async calculateMarketSentiment(asset: Asset): Promise<'bullish' | 'bearish' | 'neutral'> {
  ...
  private isContrarianTrade(order: Order, sentiment: 'bullish' | 'bearish' | 'neutral'): boolean {
  ...
  private categorizeAsset(asset: Asset): string {
    const title = asset.name.toLowerCase();
    const description = asset.description?.toLowerCase() || '';
    
    if (title.includes('hero') || title.includes('captain') || title.includes('wonder')) return 'character';
    if (title.includes('magic') || title.includes('spell') || title.includes('mystic')) return 'mystery';
    if (title.includes('empire') || title.includes('king') || title.includes('throne')) return 'power';
    if (title.includes('fire') || title.includes('water') || title.includes('earth') || title.includes('air')) return 'elemental';
    if (title.includes('ancient') || title.includes('time') || title.includes('eternal')) return 'temporal';
    if (title.includes('team') || title.includes('league') || title.includes('community')) return 'social';
    if (title.includes('knowledge') || title.includes('book') || title.includes('wisdom')) return 'educational';
    
    return 'general';
  }

  private getHouseAssetBonus(houseId: string, assetCategory: string): number {
  ...
  private determineKarmaActionType(order: Order, karmaImpact: number): 
    'aggressive_trade' | 'patient_trade' | 'calculated_investment' | 'risky_speculation' {
    if (order.orderType === 'market' && parseFloat(order.quantity.toString()) > 100) {
   ...
   */
  async getCurrentTradingModifiers(userId: string): Promise<{
    activeModifiers: TradingConsequence[];
    alignmentEffects: any;
    houseEffects: any;
    karmaEffects: any;
  }> {
    const user = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    ...
    const activeConsequences = await karmicAlignmentService.getActiveTradingConsequences(userId);
    ...
    const houseEffects = currentUser.houseId ? 
      this.HOUSE_TRADING_EFFECTS[currentUser.houseId as keyof typeof this.HOUSE_TRADING_EFFECTS] : null;
...
export const tradingConsequencesService = new TradingConsequencesService();