...
export interface TierFeatures {
  ...
  aiMarketBriefs: boolean;
  competitionAccess: boolean;
  gradedMarketplace: boolean;
  advancedCharting: boolean;
  prioritySupport: boolean;
  coursesIncluded: number;
  monthlyTradingCredits: number;
  ppixIndexAccess: boolean;
  apiAccess: boolean;
  customAlerts: number;
}
...
export class SubscriptionService {
  ...
  static hasFeatureAccess(user: User, feature: keyof TierFeatures): boolean {
  ...
  static canCreateMore(user: User, resourceType: 'watchlists' | 'portfolios', currentCount: number): boolean {
    const tier = user.subscriptionTier as SubscriptionTier;
    const features = tierFeatures[tier];
  ...
  static getRemainingCredits(user: User): number {
  ...
  static canMakeTrade(user: User, creditsCost: number = 1): boolean {
    // Elite users have unlimited trades
    if (user.subscriptionTier === 'elite') return true;
  ...
  static getUpgradeRecommendations(user: User, requestedFeature: keyof TierFeatures): {
    ...
    for (let i = currentTierIndex + 1; i < tierOrder.length; i++) {
      ...
      if (tierFeatures[tier][requestedFeature]) {
        return {
  ...
  static getTierBenefits(targetTier: SubscriptionTier, currentTier: SubscriptionTier): string[] {
    const targetFeatures = tierFeatures[targetTier];
  ...
  static getFeatureGateMessage(feature: keyof TierFeatures, recommendedTier: SubscriptionTier): string {
