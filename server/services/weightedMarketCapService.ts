...
import { goCollectService, type CensusGradeData } from './goCollectService';

interface WeightedPricingResult {
  sharePrice: number;
...
class WeightedMarketCapService {
   ...
   */
  private gradeMapping: Array<{ grade: string; pcGrade: string }> = [
   ...
   */
  async calculateWeightedPricing(
    comicName: string,
    sharesPerCopy: number = 100
  ): Promise<WeightedPricingResult | null> {
    console.log(`\n💎 Calculating weighted market cap for: ${comicName}`);

    // Step 1: Get real prices by grade from PriceCharting
    const priceData = await priceChartingService.getComicPricesByGrade(comicName);
    if (!priceData) {
    ...
    let censusData = await goCollectService.getCensusDataByName(comicName);
    ...
    if (!censusData || censusData.gradeDistribution.length === 0) {
      ...
      const distribution = goCollectService.estimateCensusDistribution(era, estimatedTotalCopies);
    ...
    const pricesByGrade = new Map<string, number>();
    ...
    const marketCap = goCollectService.calculateWeightedMarketCap(
      censusData.gradeDistribution,
      pricesByGrade,
      sharesPerCopy
    );
   ...
   */
  private calculateScarcityModifier(totalCopies: number, highestPrice: number): number {
   ...
   */
  private getPriceTier(highestPrice: number): 'golden' | 'silver' | 'bronze' | 'modern' {
    if (highestPrice > 100000) return 'golden';   // $100k+ = Golden Age
    if (highestPrice > 10000) return 'silver';    // $10k-$100k = Silver Age
    if (highestPrice > 1000) return 'bronze';     // $1k-$10k = Bronze Age
   ...
   */
  private getMedianCensusForTier(tier: 'golden' | 'silver' | 'bronze' | 'modern'): number {
    const medianCounts = {
   ...
   */
  private estimateTotalCopies(highestPrice: number, era: 'golden' | 'silver' | 'bronze' | 'modern'): number {
    ...
    if (highestPrice > 100000) return Math.floor(base * 0.5);  // Super rare
    if (highestPrice > 50000) return Math.floor(base * 0.7);
    if (highestPrice > 10000) return Math.floor(base * 0.9);
    if (highestPrice > 1000) return base;
   ...
   */
  async batchCalculateWeightedPricing(
    comicNames: string[],
    sharesPerCopy: number = 100
  ): Promise<Map<string, WeightedPricingResult>> {
    const results = new Map<string, WeightedPricingResult>();
...
export const weightedMarketCapService = new WeightedMarketCapService();
