...
export interface PersonalityConfig {
  ...
  positionSizingRange: [number, number]; // % of capital
  holdingPeriodRange: [number, number]; // days
  tradingFrequency: 'very_high' | 'high' | 'medium' | 'low' | 'very_low';
  preferredAssetTypes: string[]; // 'characters', 'issues', 'creators', 'publishers', 'options', 'bonds', 'etfs'
  stopLossRange: [number, number]; // %
  takeProfitRange: [number, number]; // %
  panicThresholdRange: [number, number];
  greedThresholdRange: [number, number];
  fomoSusceptibilityRange: [number, number];
  newsReaction: 'ignore' | 'consider' | 'emotional';
  lossCutSpeed: 'instant' | 'slow' | 'never';
}

export interface GeneratedPersonality extends PersonalityConfig {
 ...
 */
const PERSONALITY_ARCHETYPES: Record<string, PersonalityConfig> = {
  whale: {
    archetype: 'whale',
 ...
 */
export function getArchetypeBehavior(archetype: string): PersonalityConfig {
...
export function generatePersonalityConfig(
  ...
  const generated: GeneratedPersonality = {
    ...config,
 ...
 */
export function generateNPCPopulation(
  count: number,
  archetypes?: string[]
): GeneratedPersonality[] {
  const availableArchetypes = archetypes || getAllArchetypes();
  const population: GeneratedPersonality[] = [];
 ...
 */
export function getTradingFrequencyValue(frequency: PersonalityConfig['tradingFrequency']): number {
  const frequencyMap = {
 ...
 */
export function shouldNPCTrade(
 ...
 * @returns 'buy' | 'sell'
 */
export function getNPCTradeDirection(
  personality: GeneratedPersonality,
  marketSentiment: number,
  priceChange: number,
  newsImpact: number = 0
 ...
 */
export function calculateNPCPositionSize(
  personality: GeneratedPersonality,
