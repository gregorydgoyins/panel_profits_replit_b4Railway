...
import type {
  GradedAssetProfile,
  InsertGradedAssetProfile,
  Asset,
  MarketData,
  MarketComparable,
  InsertMarketComparable,
  GradingCertification,
  InsertGradingCertification
...
export interface MarketDemandFactors {
  ...
  keyIssueStatus: number;
  creatorSignificance: number;
  houseAffiliation: number;
}

// Grading assessment interface
...
export class GradingEvaluationService {
  constructor(private storage: DatabaseStorage) {}

  /**
   * Perform comprehensive CGC-style grading assessment
   */
  async performGradingAssessment(
    assetId: string,
    gradingCriteria: {
      condition: number;
      centering: number;
    ...
    certificationAuthority: string = 'internal',
    additionalNotes?: string
  ): Promise<GradingAssessment> {
   ...
   */
  async calculateRarityAnalysis(
    assetId: string,
    marketFactors?: Partial<MarketDemandFactors>
   ...
   */
  async createGradedAssetProfile(
    userId: string,
    assetId: string,
    ...
    const gradedProfileData: InsertGradedAssetProfile = {
      userId,
      assetId,
      ...
      houseProgressionValue: this.calculateHouseProgressionValue(
        gradingAssessment.overallGrade,
        rarityAnalysis.rarityScore
      ).toString()
    };
    ...
    const certificationData: InsertGradingCertification = {
  ...
  async performReGrading(
    ...
    const newAssessment = await this.performGradingAssessment(
      existingProfile.assetId,
      newGradingCriteria,
    ...
    const reGradingData: InsertGradingCertification = {
      gradedAssetId,
  ...
  private calculateOverallGrade(scores: {
    condition: number;
    centering: number;
    corners: number;
    edges: number;
  ...
  private generateGradingNotes(gradingData: {
    condition: number;
    centering: number;
    corners: number;
    edges: number;
  ...
  private calculateAssessmentConfidence(scores: number[]): number {
  ...
  private calculateScarcityFactors(asset: Asset, marketData: MarketData | null, marketHistory: MarketData[]) {
  ...
  private calculateVariantRarity(asset: Asset): number {
    ...
    return variantScores[variantType as keyof typeof variantScores] || 0;
  }

  private calculateMarketPresence(marketHistory: MarketData[]): number {
  ...
  private calculateMarketDemandScore(
    ...
    factors?: Partial<MarketDemandFactors>
  ): number {
    let demandScore = 0;
  ...
  private calculateRarityScore(scarcityFactors: any, marketDemandScore: number): number {
    const scarcityScore = (
      scarcityFactors.ageBonus +
  ...
  private determineRarityTier(rarityScore: number): string {
    if (rarityScore >= RARITY_THRESHOLDS.MYTHIC) return 'mythic';
    if (rarityScore >= RARITY_THRESHOLDS.LEGENDARY) return 'legendary';
    if (rarityScore >= RARITY_THRESHOLDS.ULTRA_RARE) return 'ultra_rare';
    if (rarityScore >= RARITY_THRESHOLDS.RARE) return 'rare';
    if (rarityScore >= RARITY_THRESHOLDS.UNCOMMON) return 'uncommon';
    return 'common';
  }

  private async findMarketComparables(assetId: string): Promise<MarketComparable[]> {
    // This would typically query external market data sources
    // For now, return empty array - implement with real market data
    return [];
  }

  private calculateHouseProgressionValue(grade: number, rarityScore: number): number {
