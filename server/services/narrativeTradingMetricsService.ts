...
import { 
  narrativeTradingMetrics, houseFinancialProfiles, storyEventTriggers, narrativeMarketEvents,
  enhancedCharacters, battleScenarios, enhancedComicIssues, moviePerformanceData, 
  assetFinancialMapping, assets, marketInsights, marketData, narrativeTimelines, storyBeats,
  assetCurrentPrices, marketEvents
} from '@shared/schema.js';
import type {
  InsertNarrativeTradingMetrics, InsertHouseFinancialProfile, InsertStoryEventTrigger,
  InsertNarrativeMarketEvent, NarrativeTradingMetrics, HouseFinancialProfile,
  StoryEventTrigger, NarrativeMarketEvent, EnhancedCharacter, Asset, MarketInsight,
  InsertMarketInsight, StoryBeat, NarrativeTimeline, AssetCurrentPrice
} from '@shared/schema.js';
 ...
 */
export class NarrativeTradingMetricsService {
  ...
  private readonly POWER_LEVEL_FACTORS = {
  ...
  private readonly STORY_BEAT_IMPACTS = {
   ...
   */
  async initializeHouseFinancialProfiles(): Promise<void> {
    ...
    for (const [houseId, profile] of Object.entries(this.HOUSE_THEMES)) {
      try {
        ...
        const existingProfile = await db.select()
          .from(houseFinancialProfiles)
          .where(eq(houseFinancialProfiles.houseId, houseId))
          .limit(1);
        ...
        const houseProfile: InsertHouseFinancialProfile = {
          houseId: houseId,
        ...
        await db.insert(houseFinancialProfiles).values(houseProfile);
   ...
   */
  async calculateMythicVolatility(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
   ...
   */
  async calculateNarrativeMomentum(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
   ...
   */
  async generateTradingMetrics(assetId: string): Promise<InsertNarrativeTradingMetrics> {
    try {
      // Get asset and character data
      const asset = await db.select().from(assets).where(eq(assets.id, assetId)).limit(1);
      if (!asset[0]) throw new Error(`Asset ${assetId} not found`);

      let characterData: EnhancedCharacter | undefined;
      if (asset[0].type === 'character') {
        const characters = await db.select().from(enhancedCharacters)
          .where(eq(enhancedCharacters.name, asset[0].name))
          .limit(1);
      ...
      const tradingMetrics: InsertNarrativeTradingMetrics = {
        assetId: assetId,
   ...
   */
  async processStoryEventTriggers(storyBeat: StoryBeat): Promise<string[]> {
    ...
    try {
      ...
      const trigger: InsertStoryEventTrigger = {
      ...
      const [insertedTrigger] = await db.insert(storyEventTriggers).values(trigger).returning();
   ...
   */
  async generateNarrativeMarketInsights(assetId: string): Promise<InsertMarketInsight[]> {
    const insights: InsertMarketInsight[] = [];
    
    try {
      // Get asset and its narrative trading metrics
      const asset = await db.select().from(assets).where(eq(assets.id, assetId)).limit(1);
      if (!asset[0]) return insights;

      const metrics = await db.select().from(narrativeTradingMetrics)
        .where(eq(narrativeTradingMetrics.assetId, assetId))
        .limit(1);

      if (!metrics[0]) return insights;
   ...
   */
  async updateAllTradingMetrics(): Promise<number> {
    let updatedCount = 0;
        ...
        for (const asset of batch) {
          try {
            ...
            const existingMetrics = await db.select()
              ...
              .where(eq(narrativeTradingMetrics.assetId, asset.id))
              .limit(1);
  ...
  private async determineHouseAffiliation(asset: Asset, characterData?: EnhancedCharacter): Promise<string | null> {
  ...
  private async determineStoryArcPhase(asset: Asset): Promise<string> {
    // Analyze recent story beats to determine current phase
    try {
      const recentTimelines = await db.select()
        .from(narrativeTimelines)
        .where(eq(narrativeTimelines.primaryCharacter, asset.name))
        .orderBy(desc(narrativeTimelines.createdAt))
        .limit(3);
  ...
  private getStoryArcVolatilityMultiplier(phase: string): number {
    const multipliers = {
      'origin': 1.2,
      'rising_action': 1.1,
      'climax': 1.8,
  ...
  private async calculateCosmicEventBoost(asset: Asset): Promise<number> {
    // Check for recent cosmic-level events affecting this asset
    try {
      const recentCosmicEvents = await db.select()
        .from(storyBeats)
        .where(and(
          eq(storyBeats.beatType, 'cosmic_event'),
          gte(storyBeats.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) // Last 7 days
        ))
        .limit(5);
  ...
  private async calculateCulturalImpact(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
  ...
  private async calculateStoryProgressionRate(asset: Asset): Promise<number> {
    // Analyze frequency of story beats for this asset
    try {
      const recentBeats = await db.select()
        .from(storyBeats)
        .where(and(
          eq(storyBeats.primaryCharacterId, asset.id),
          gte(storyBeats.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)) // Last 30 days
        ));
  ...
  private async calculateThemeRelevance(asset: Asset): Promise<number> {
  ...
  private async calculateMediaBoost(asset: Asset): Promise<number> {
    // Check for recent movie/TV show releases
    try {
      const recentMovies = await db.select()
        .from(moviePerformanceData)
        .where(and(
          eq(moviePerformanceData.primaryCharacter, asset.name),
          gte(moviePerformanceData.releaseDate, new Date(Date.now() - 180 * 24 * 60 * 60 * 1000)) // Last 6 months
        ));
  ...
  private async calculateMomentumDecay(asset: Asset): Promise<number> {
    ...
    try {
      ...
      if (!lastEvent) return 0.95; // Slight decay if no events
  ...
  private calculateHouseSpecialtyBonus(asset: Asset, houseProfile: any): number {
    if (!houseProfile) return 0.0;
    ...
    if (isSpecialty) return houseProfile.tradingBonusPercentage;
    if (isWeakness) return -houseProfile.penaltyPercentage;
    
    return 0.0;
  }

  private async calculateNarrativeMarginRequirement(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
    let baseMargin = 50.0;
  ...
  private async calculateStoryRiskAdjustment(asset: Asset): Promise<number> {
  ...
  private generateSeasonalityPattern(houseId: string): any {
  ...
  private generateEventResponseProfile(houseId: string): any {
  ...
  private generateStoryBeatMultipliers(houseId: string): any {
  ...
  private generatePowerLevelWeights(houseId: string): any {
  ...
  private generateAlignmentRequirements(houseId: string): any {
    const requirements = {
  ...
  private getSynergisticHouses(houseId: string): string[] {
    const synergies = {
  ...
  private getConflictingHouses(houseId: string): string[] {
    const conflicts = {
  ...
  private generateVolatilityInsight(asset: Asset, metrics: NarrativeTradingMetrics): InsertMarketInsight {
  ...
  private generateMomentumInsight(asset: Asset, metrics: NarrativeTradingMetrics): InsertMarketInsight {
  ...
  private generateHouseSpecificInsight(asset: Asset, metrics: NarrativeTradingMetrics): InsertMarketInsight {
  ...
  private generateStoryArcInsight(asset: Asset, metrics: NarrativeTradingMetrics): InsertMarketInsight {
  ...
  private getPhaseCategory(phase: string): string {
    const categories = {
      'origin': 'bullish',
  ...
  private getPhasesentiment(phase: string): number {
    const sentiments = {
      'origin': 0.3,
      'rising_action': 0.5,
  ...
  private async getLastNarrativeEvent(assetId: string): Promise<Date | null> {
    try {
      const lastEvent = await db.select()
        .from(storyBeats)
        .where(eq(storyBeats.primaryCharacterId, assetId))
        .orderBy(desc(storyBeats.createdAt))
        .limit(1);
  ...
  private async predictNextNarrativeEvent(assetId: string): Promise<Date | null> {
  ...
  private generateSeasonalNarrativePattern(asset: Asset): number[] {
    // Generate 12-month narrative pattern (simplified)
    return Array.from({ length: 12 }, (_, i) => 
      0.8 + 0.4 * Math.sin((i + 3) * Math.PI / 6) // Seasonal wave pattern
    );
  }

  private async calculateMetricsReliability(assetId: string): Promise<number> {
  ...
  private async calculatePredictionAccuracy(assetId: string): Promise<number> {
  ...
  private assessEventSeverity(storyBeat: StoryBeat): string {
  ...
  private classifyTriggerType(storyBeat: StoryBeat): string {
    if (storyBeat.beatType.includes('cosmic')) return 'cosmic_event';
    if (storyBeat.beatType.includes('character')) return 'character_event';
    if (storyBeat.mediaReferences && storyBeat.mediaReferences.length > 0) return 'media_release';
    return 'story_beat';
  }

  private async getAffectedAssets(storyBeat: StoryBeat): Promise<{ direct: string[], indirect: string[] }> {
    const direct: string[] = [];
  ...
  private calculateHouseResponseMultipliers(storyBeat: StoryBeat): any {
    const multipliers = {};
  ...
  private calculateCrossHouseEffects(storyBeat: StoryBeat): any {
  ...
  private determineAffectedAssetTypes(storyBeat: StoryBeat): string[] {
  ...
  private calculateImpactDuration(severity: string, term: string): number {
    const durations = {
      immediate: {
        'minor': 60,        // 1 hour
  ...
  private calculateMemoryDecay(severity: string): number {
    const decayRates = {
  ...
  private generateTriggerConditions(storyBeat: StoryBeat): any {
    return {
  ...
  private calculateCooldownPeriod(severity: string): number {
    const cooldowns = {
      'minor': 60,           // 1 hour
      'moderate': 240,       // 4 hours
      'major': 720,          // 12 hours
  ...
  private calculateMaxActivations(severity: string): number {
  ...
  private async calculateStoryBeatSensitivity(asset: Asset): Promise<number> {
  ...
  private async calculateCharacterDeathImpact(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
    if (!characterData) return 0.0;
  ...
  private async calculatePowerUpgradeImpact(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
    if (!characterData) return 0.0;
  ...
  private async calculateResurrectionImpact(asset: Asset, characterData?: EnhancedCharacter): Promise<number> {
    if (!characterData) return 0.0;
  ...
  private async calculateNarrativeCorrelation(asset: Asset): Promise<number> {
...
export const narrativeTradingMetricsService = new NarrativeTradingMetricsService();