...
interface CreatorMetadata {
  name: string;
  roles?: string[]; // ['writer', 'artist', 'cover', 'editorial']
  appearances?: number;
  keyIssues?: string[];
  notableWorks?: string[];
}

type CharacterTier = 1 | 2 | 3 | 4;
type CreatorTier = 1 | 2 | 3 | 4;
...
class TierClassificationService {
   ...
   */
  private readonly tier1Characters = new Set([
    // DC Flagship
   ...
   */
  private readonly tier2Characters = new Set([
   ...
   */
  private readonly tier3Characters = new Set([
   ...
   */
  private readonly tier1Creators = new Set([
   ...
   */
  private readonly tier2Creators = new Set([
   ...
   * Top artists (Tier 3)
   */
  private readonly tier3Creators = new Set([
   ...
   */
  classifyCharacter(metadata: CharacterMetadata): {
    tier: CharacterTier;
    isVariant: boolean;
    confidence: number;
  } {
    ...
    if (isVariant && appearances > 100) return { tier: 2, isVariant, confidence: 0.7 };
    
    if (appearances > 500) return { tier: 1, isVariant: false, confidence: 0.7 };
    if (appearances > 200) return { tier: 2, isVariant, confidence: 0.7 };
    if (appearances > 50) return { tier: 3, isVariant, confidence: 0.6 };
   ...
   */
  classifyCreator(metadata: CreatorMetadata): {
    tier: CreatorTier;
    ...
    // Fallback to appearance-based classification
    if (roleWeightedAppearances > 300) return { tier: 1, roleWeightedAppearances, confidence: 0.7 };
    if (roleWeightedAppearances > 100) return { tier: 2, roleWeightedAppearances, confidence: 0.7 };
    if (roleWeightedAppearances > 30) return { tier: 3, roleWeightedAppearances, confidence: 0.6 };
   ...
   */
  private calculateRoleWeightedAppearances(metadata: CreatorMetadata): number {
    const appearances = metadata.appearances || 0;
   ...
   */
  extractVariantInfo(name: string): {
    baseName: string;
    variant: string | null;
  } {
   ...
   */
  batchClassifyCharacters(characters: CharacterMetadata[]): Map<string, {
    tier: CharacterTier;
    isVariant: boolean;
    confidence: number;
  }> {
   ...
   */
  batchClassifyCreators(creators: CreatorMetadata[]): Map<string, {
    tier: CreatorTier;
    roleWeightedAppearances: number;
    confidence: number;
  }> {
...
export const tierClassificationService = new TierClassificationService();
export type { CharacterMetadata, CreatorMetadata, CharacterTier, CreatorTier };
