...
import type {
  Trade, TradingVictim, User, MoralStanding,
  InsertJournalEntry, InsertPsychologicalProfile,
  JournalEntry, PsychologicalProfile
} from '@shared/schema';

interface JournalGenerationContext {
  trade?: Trade;
...
interface PsychologyAnalysisResult {
  ...
  tradingStyle: string;
  empathyScore: number;
  ruthlessnessIndex: number;
  denialLevel: number;
  turningPoints: any[];
}

export class NoirJournalService {
  private openai: OpenAI;
  private static instance: NoirJournalService;
  ...
  static getInstance(): NoirJournalService {
   ...
   */
  async generateNoirEntry(context: JournalGenerationContext): Promise<JournalEntry> {
    try {
      ...
      const result = JSON.parse(response.choices[0].message.content || '{}') as NoirPromptResult;
      ...
      return await storage.createJournalEntry(entry);
    } catch (error) {
   ...
   */
  async analyzeTraderPsychology(
    userId: string,
    tradingHistory: Trade[],
    victims: TradingVictim[],
    ...
    try {
      ...
      const analysis = JSON.parse(response.choices[0].message.content || '{}') as PsychologyAnalysisResult;

      // Check if profile exists
      const existingProfile = await storage.getPsychologicalProfile(userId);
      
      if (existingProfile) {
        // Update existing profile
        return await storage.updatePsychologicalProfile(userId, {
        ...
        // Create new profile
        const profileData: InsertPsychologicalProfile = {
          userId,
          pattern: analysis.pattern,
          analysis: analysis.analysis,
          dominantTraits: analysis.dominantTraits,
        ...
        return await storage.createPsychologicalProfile(profileData);
      }
    } catch (error) {
   ...
   */
  async writeCorruptionNarrative(
    userId: string,
    corruptionLevel: number,
    milestone: number
   ...
   */
  async generateShadowConfession(
    userId: string,
    shadowActivity: any,
    corruptionLevel: number
  ): Promise<JournalEntry> {
   ...
   */
  private buildNoirPrompt(context: JournalGenerationContext): string {
   ...
   */
  private determineEntryType(context: JournalGenerationContext): string {
    if (context.victim) return 'victim';
    if (context.trade) return 'trade';
    return 'observation';
  }
}

// Export singleton instance
export const noirJournalService = NoirJournalService.getInstance();