...
import {
  users, detailedKarmaActions, alignmentHistory, tradingConsequences,
  alignmentThresholds, karmicProfiles, orders, assets
} from '@shared/schema.js';
import type {
  User, InsertDetailedKarmaAction, InsertAlignmentHistory, InsertTradingConsequence,
  InsertKarmicProfile, AlignmentThreshold, KarmicProfile, Order
} from '@shared/schema.js';
 ...
 */
export class KarmicAlignmentService {
  
  // House-specific karma multipliers and alignment influences
  private readonly HOUSE_KARMA_MODIFIERS = {
    heroes: {
      karmaMultiplier: 1.2,
  ...
  private readonly ACTION_KARMA_IMPACTS = {
   ...
   */
  async recordKarmaAction(
    userId: string,
    actionType: keyof typeof this.ACTION_KARMA_IMPACTS,
    context: {
      assetId?: string;
      orderId?: string;
    ...
    const houseModifiers = currentUser.houseId ? this.HOUSE_KARMA_MODIFIERS[currentUser.houseId as keyof typeof this.HOUSE_KARMA_MODIFIERS] : null;
    ...
    const karmaAction: InsertDetailedKarmaAction = {
      userId,
      actionType,
    ...
    await db.insert(detailedKarmaActions).values(karmaAction);
   ...
   */
  private async updateUserAlignment(
    userId: string,
    karmaChange: number,
    ...
    const alignmentShift = Math.sqrt(
      Math.pow(newLawfulChaotic - currentLC, 2) + Math.pow(newGoodEvil - currentGE, 2)
    );
   ...
   */
  private async recordAlignmentHistory(userId: string, historyData: Partial<InsertAlignmentHistory>): Promise<void> {
    const alignmentRecord: InsertAlignmentHistory = {
      userId,
    ...
    await db.insert(alignmentHistory).values(alignmentRecord);
  }

  /**
   * Check if user has crossed any alignment thresholds
   */
  private async checkAlignmentThresholds(userId: string): Promise<void> {
    const user = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    if (!user[0]) return;
    ...
    const thresholds = await db.select()
      .from(alignmentThresholds)
      .where(eq(alignmentThresholds.isActive, true));
   ...
   */
  private async applyAlignmentThreshold(userId: string, threshold: AlignmentThreshold): Promise<void> {
    const user = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    if (!user[0]) return;

    const tradingBonuses = threshold.tradingBonuses as any;
    if (!tradingBonuses) return;

    // Create trading consequences for each bonus
    for (const [bonusType, bonusValue] of Object.entries(tradingBonuses)) {
      const consequence: InsertTradingConsequence = {
        userId,
        ...
        modifierValue: (bonusValue as number).toString(),
      ...
      await db.insert(tradingConsequences).values(consequence);
    }
  }
   ...
   */
  private async updateKarmicProfile(userId: string): Promise<void> {
    const user = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    if (!user[0]) return;
    ...
    const recentActions = await db.select()
      .from(detailedKarmaActions)
      .where(eq(detailedKarmaActions.userId, userId))
      .orderBy(desc(detailedKarmaActions.createdAt))
      .limit(50);
    ...
    const profileData: InsertKarmicProfile = {
      userId,
    ...
    const existingProfile = await db.select()
      .from(karmicProfiles)
      .where(eq(karmicProfiles.userId, userId))
      .limit(1);

    if (existingProfile[0]) {
      await db.update(karmicProfiles)
        .set({ ...profileData, updatedAt: new Date() })
        .where(eq(karmicProfiles.userId, userId));
    } else {
      await db.insert(karmicProfiles).values(profileData);
    }
  }

  // Helper methods for calculations and analysis

  private determineBehaviorPattern(actionType: string, context: any) {
    const patterns = {
      trading: 'balanced',
      community: 'neutral', 
      risk: 'moderate'
  ...
  private getActionSubtype(actionType: string): string {
    return actionType.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  }

  private generateMysticalDescription(actionType: string, houseId: string | null, severity: string): string {
    ...
    const houseDesc = houseId ? houseDescriptions[houseId as keyof typeof houseDescriptions] : "Mystical energies swirl around your unaligned soul";
    const severityDesc = severityDescriptions[severity as keyof typeof severityDescriptions];
  ...
  private determineAlignmentPhase(shiftMagnitude: number): string {
    if (shiftMagnitude < 10) return 'awakening';
    if (shiftMagnitude < 25) return 'journey';
    if (shiftMagnitude < 50) return 'transformation';
    return 'mastery';
  }

  private determineSignificanceLevel(shiftMagnitude: number): string {
    if (shiftMagnitude < 5) return 'minor';
    if (shiftMagnitude < 15) return 'moderate';
    if (shiftMagnitude < 30) return 'major';
    if (shiftMagnitude < 50) return 'critical';
    return 'legendary';
  }

  private generateCosmicEvent(): string {
    const events = [
  ...
  private generateProphecy(phase: string): string {
    const prophecies = {
    ...
    return prophecies[phase as keyof typeof prophecies] || prophecies.journey;
  }

  private isWithinRange(value: number, min: string | null, max: string | null): boolean {
    if (min !== null && value < parseFloat(min)) return false;
    if (max !== null && value > parseFloat(max)) return false;
    return true;
  }
  ...
  private calculateKarmaStats(actions: any[]) {
  ...
  private async analyzeAlignmentTrend(userId: string) {
    const history = await db.select()
      .from(alignmentHistory)
      .where(eq(alignmentHistory.userId, userId))
      .orderBy(desc(alignmentHistory.recordedAt))
      .limit(10);
  ...
  private calculateAlignmentTrend(history: any[]): string {
    ...
    if (recentMagnitude > olderMagnitude * 1.2) return 'ascending';
    if (recentMagnitude < olderMagnitude * 0.8) return 'descending';
    return 'stable';
  }

  private predictAlignmentDirection(history: any[]): string {
  ...
  private determineTradingPersonality(analysis: any): string {
    ...
    if (dominant === 'patient' && riskProfile === 'conservative') return 'patient_strategist';
    if (dominant === 'aggressive' && riskProfile === 'reckless') return 'aggressive_opportunist';
    if (socialBehavior === 'helpful') return 'community_leader';
    if (socialBehavior === 'competitive') return 'competitive_trader';
    return 'balanced_explorer';
  }

  private calculateHouseCompatibility(user: User): number {
    if (!user.houseId) return 50.0;
    
    const houseModifiers = this.HOUSE_KARMA_MODIFIERS[user.houseId as keyof typeof this.HOUSE_KARMA_MODIFIERS];
  ...
  private determineOptimalHouse(user: User): string {
    const userLC = parseFloat(user.lawfulChaoticAlignment || '0');
    const userGE = parseFloat(user.goodEvilAlignment || '0');
  ...
  private determineConflictLevel(user: User): string {
    const compatibility = this.calculateHouseCompatibility(user);
    
    if (compatibility >= 80) return 'none';
    if (compatibility >= 60) return 'minor';
    if (compatibility >= 40) return 'moderate';
    return 'severe';
  }

  private calculateCosmicResonance(user: User): number {
    const karma = user.karma || 0;
    const alignment = Math.sqrt(
      Math.pow(parseFloat(user.lawfulChaoticAlignment || '0'), 2) + 
  ...
  private calculateDivineFavor(user: User): number {
    const karma = user.karma || 0;
  ...
  private calculateShadowInfluence(user: User): number {
    const karma = user.karma || 0;
  ...
  private getSecondMostCommon(arr: string[]): string | null {
  ...
  private calculateConsistency(patterns: string[]): number {
    if (patterns.length === 0) return 50;
    const mostCommon = this.getMostCommon(patterns);
    if (!mostCommon) return 50;
   ...
   */
  async getUserKarmicProfile(userId: string): Promise<KarmicProfile | null> {
   ...
   */
  async getUserAlignmentHistory(userId: string, limit: number = 20) {
   ...
   */
  async getActiveTradingConsequences(userId: string) {
    return await db.select()
      .from(tradingConsequences)
      .where(
        and(
          eq(tradingConsequences.userId, userId),
          eq(tradingConsequences.consequenceApplied, true)
        )
      )
      .orderBy(desc(tradingConsequences.createdAt));
  }
}

// Export singleton instance
export const karmicAlignmentService = new KarmicAlignmentService();