...
import type { 
  User, Trade, MoralStanding, TradingVictim, 
  InsertMoralStanding, InsertTradingVictim 
} from "@shared/schema";
...
export class MoralConsequenceEngine {
  private static instance: MoralConsequenceEngine;

  private constructor() {}

  static getInstance(): MoralConsequenceEngine {
  ...
  async generateVictim(trade: Trade, profit: number): Promise<TradingVictim> {
    ...
    return await storage.createVictim(victim);
  }

  /**
   * Generate a realistic victim name
   */
  generateVictimName(): string {
   ...
   */
  private determineImpactLevel(lossAmount: number): 'minor' | 'moderate' | 'severe' | 'catastrophic' {
    if (lossAmount < 1000) return 'minor';
    if (lossAmount < 10000) return 'moderate';
    if (lossAmount < 50000) return 'severe';
    return 'catastrophic';
  }
   ...
   */
  generateImpactStory(lossAmount: number, impactLevel: 'minor' | 'moderate' | 'severe' | 'catastrophic'): string {
   ...
   */
  private getConsequence(impactLevel: 'minor' | 'moderate' | 'severe' | 'catastrophic'): string {
   ...
   */
  async calculateCorruption(userId: string, profit: number): Promise<number> {
    ...
    let moralStanding = await storage.getMoralStanding(userId);
    
    if (!moralStanding) {
      // Initialize moral standing for new users
      const newStanding: InsertMoralStanding = {
        userId,
      ...
      moralStanding = await storage.createMoralStanding(newStanding);
    } else {
      // Update existing moral standing
      const newCorruption = Math.min(
        parseFloat(moralStanding.corruptionLevel || '0') + scaledCorruption, 
        100
      );
   ...
   */
  private getSoulWeight(corruptionLevel: number): string {
    if (corruptionLevel < 20) return 'unburdened';
    if (corruptionLevel < 40) return 'tainted';
    if (corruptionLevel < 60) return 'heavy';
    if (corruptionLevel < 80) return 'crushing';
    return 'damned';
  }
   ...
   */
  async trackBloodMoney(userId: string, amount: number): Promise<void> {
   ...
   */
  async processConfession(userId: string): Promise<{ 
    success: boolean; 
    message: string;
   ...
   */
  async getMoralState(userId: string): Promise<{
    corruption: number;
    soulWeight: string;
    victimCount: number;
    bloodMoney: number;
...
export const moralConsequenceEngine = MoralConsequenceEngine.getInstance();