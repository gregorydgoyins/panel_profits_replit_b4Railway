...
import type { 
  Position, Trade, InsertPosition, AssetCurrentPrice,
  User, Portfolio
} from "@shared/schema";
...
export class PositionService {
  ...
  private async handleBuyTrade(
    ...
    if (existingPosition) {
      ...
      const createdPosition = await storage.createPosition(newPosition);
      return {
   ...
   */
  async handlePositionAveraging(
    position: Position,
    newQuantity: number,
    newPrice: number
  ): Promise<{ newAverageCost: number; totalCostBasis: number }> {
   ...
   */
  getHoldingPeriod(position: Position): {
    days: number;
    isLongTerm: boolean;
    taxRate: number;
  } {
  ...
  async checkRiskLevels(
    userId: string,
    portfolioId: string
  ): Promise<{ stopLossHit: Position[], takeProfitHit: Position[] }> {
    const positions = await storage.getPositions(userId, portfolioId);
    const stopLossHit: Position[] = [];
    const takeProfitHit: Position[] = [];
   ...
   */
  async updateMaxUnrealizedProfit(position: Position): Promise<void> {
    const currentPrice = await storage.getAssetCurrentPrice(position.assetId);
  ...
  async getConcentrationRisk(
    ...
    topPositions: Array<{ assetId: string; percentage: number }>;
