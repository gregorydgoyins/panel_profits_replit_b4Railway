import { scholarlyInvestmentFramework } from './scholarlyInvestmentFramework.js';
import type { ComicInvestmentEvaluation } from './scholarlyInvestmentFramework.js';

// Define TradingAsset interface locally
...
/**
 * Scholarly PPIx Service
 * Implements PPIx 50 and PPIx 100 indices using the 20 Art Investment Metrics
 * This removes all fan bias and creates institutional-grade index selection
 */

export interface ScholarlyPPIxIndex {
  name: string;
  description: string;
  currentValue: number;
  dayChange: number;
...
class ScholarlyPPIxService {
   ...
   */
  generateScholarlyDemoAssets(): TradingAsset[] {
    return [
   ...
   */
  calculateScholarlyPPIx50(): ScholarlyPPIxIndex {
    const assets = this.generateScholarlyDemoAssets();
    const evaluations = scholarlyInvestmentFramework.generatePPIx50Constituents(assets);
   ...
   */
  calculateScholarlyPPIx100(): ScholarlyPPIxIndex {
    const assets = this.generateScholarlyDemoAssets();
    const evaluations = scholarlyInvestmentFramework.generatePPIx100Constituents(assets);
  ...
  private getTopMetrics(evaluation: ComicInvestmentEvaluation): string[] {
  ...
  private getPPIx50Methodology(): string {
  ...
  private getPPIx50SelectionCriteria(): string {
  ...
  private getPPIx100Methodology(): string {
  ...
  private getPPIx100SelectionCriteria(): string {
  ...
  private getAcademicBasis(): string {
...
export const scholarlyPPIxService = new ScholarlyPPIxService();