 ...
 */
export class NostomaniaScraper extends BaseEntityScraper {
  private readonly baseUrl = 'https://www.nostomania.com';
  private readonly searchUrl = 'https://www.nostomania.com/search';
  ...
  async scrapeEntities(query?: { 
    ...
    try {
      ...
      if (query?.era) searchParams.append('era', query.era);
   ...
   */
  async scrapeVintageComics(era?: 'golden' | 'silver' | 'bronze'): Promise<EntityData[]> {
    const eraYears = {
      golden: '1938-1956',
      silver: '1956-1970',
      bronze: '1970-1985'
   ...
   */
  async scrapeHighValueComics(): Promise<EntityData[]> {
  ...
  private scrapePricingData($: cheerio.CheerioAPI): Record<string, number> {
  ...
  private determineEra(yearText: string): string {
  ...
  private generateEntityId(...parts: (string | undefined)[]): string {
    return parts
      .filter(Boolean)
      .join('-')
      .toLowerCase()
