...
import { OptionsCalculator, MarketHoursManager, ImfVaultingEngine, NpcTradingEngine, InformationTierManager, TradingFirmEngine } from './tradingEngine.js';
import { 
  insertTradingFirmSchema, insertImfVaultSettingsSchema, insertOptionsChainSchema,
  insertGlobalMarketHoursSchema, insertMarginAccountSchema, insertShortPositionSchema,
  insertNpcTraderSchema, insertInformationTierSchema, insertNewsArticleSchema,
  insertAssetFinancialMappingSchema
...
router.get('/trading-firms/:id', async (req, res) => {
  try {
    const firm = await databaseStorage.getTradingFirm(req.params.id);
    if (!firm) {
...
router.get('/trading-firms/house/:houseId', async (req, res) => {
  try {
    const firms = await databaseStorage.getTradingFirmsByHouse(req.params.houseId);
    res.json(firms);
  } catch (error) {
...
router.post('/trading-firms', async (req, res) => {
  try {
    const validatedData = insertTradingFirmSchema.parse(req.body);
    const firm = await databaseStorage.createTradingFirm(validatedData);
...
router.post('/trading-firms/:id/calculate-bonus', async (req, res) => {
  try {
    ...
    const bonus = TradingFirmEngine.calculateFirmBonus(firm, assetType, tradeSize);
    res.json(bonus);
  } catch (error) {
...
router.get('/imf-vault/:assetId', async (req, res) => {
  try {
    const vaultSettings = await databaseStorage.getImfVaultSettings(req.params.assetId);
    if (!vaultSettings) {
...
router.get('/imf-vault', async (req, res) => {
  try {
    const vaultSettings = await databaseStorage.getAllImfVaultSettings();
    res.json(vaultSettings);
  } catch (error) {
...
router.post('/imf-vault', async (req, res) => {
  try {
    const validatedData = insertImfVaultSettingsSchema.parse(req.body);
    const vaultSettings = await databaseStorage.createImfVaultSettings(validatedData);
...
router.post('/imf-vault/:assetId/scarcity', async (req, res) => {
  try {
    ...
    await databaseStorage.updateImfVaultSettings(req.params.assetId, {
...
router.post('/imf-vault/:assetId/check-vaulting', async (req, res) => {
  try {
    ...
    const shouldVault = ImfVaultingEngine.shouldTriggerVaulting(
      vaultSettings, currentPrice, marketCap, volumeRatio
    );
    
    const vaultingFee = shouldVault 
      ? ImfVaultingEngine.calculateVaultingFee(vaultSettings, 1, currentPrice)
      : 0;
...
router.get('/options/:underlyingAssetId', async (req, res) => {
  try {
    const options = await databaseStorage.getOptionsChainByUnderlying(req.params.underlyingAssetId);
    res.json(options);
  } catch (error) {
...
router.get('/options/expiration/:date', async (req, res) => {
  try {
    ...
    const options = await databaseStorage.getOptionsChainByExpiration(expirationDate);
    res.json(options);
  } catch (error) {
...
router.post('/options/calculate-pricing', async (req, res) => {
  try {
    ...
    const pricing = OptionsCalculator.calculateOptionPricing(
      underlyingPrice, strikePrice, timeToExpiration, riskFreeRate, volatility, optionType
    );
...
router.post('/options/implied-volatility', async (req, res) => {
  try {
    ...
    const impliedVolatility = OptionsCalculator.calculateImpliedVolatility(
      marketPrice, underlyingPrice, strikePrice, timeToExpiration, riskFreeRate, optionType
    );
...
router.post('/options', async (req, res) => {
  try {
    const validatedData = insertOptionsChainSchema.parse(req.body);
    const option = await databaseStorage.createOptionsChain(validatedData);
...
router.get('/market-hours/active', async (req, res) => {
  try {
    const activeMarkets = await databaseStorage.getActiveMarkets();
    
    // Add real-time market status
    const marketsWithStatus = activeMarkets.map(market => ({
      ...market,
      isCurrentlyOpen: MarketHoursManager.isMarketOpen(market),
      nextOpenTime: MarketHoursManager.getNextMarketOpen(market)
    }));
...
router.get('/market-hours/:marketCode', async (req, res) => {
  try {
    const marketHours = await databaseStorage.getGlobalMarketHours(req.params.marketCode);
    if (!marketHours) {
    ...
    const status = {
      ...marketHours,
      isCurrentlyOpen: MarketHoursManager.isMarketOpen(marketHours),
      nextOpenTime: MarketHoursManager.getNextMarketOpen(marketHours)
    };
...
router.post('/market-hours/cross-market-adjustment', async (req, res) => {
  try {
    ...
    const primaryMarket = await databaseStorage.getGlobalMarketHours(primaryMarketCode);
    const tradingMarket = await databaseStorage.getGlobalMarketHours(tradingMarketCode);
    
    if (!primaryMarket || !tradingMarket) {
      return res.status(404).json({ error: 'Market hours not found' });
    }
    
    const adjustment = MarketHoursManager.getCrossMarketAdjustment(primaryMarket, tradingMarket);
    res.json({ adjustment });
  } catch (error) {
...
router.get('/margin-accounts/user/:userId', async (req, res) => {
  try {
    const marginAccounts = await databaseStorage.getUserMarginAccounts(req.params.userId);
...
router.get('/margin-accounts/:userId/:portfolioId', async (req, res) => {
  try {
    const marginAccount = await databaseStorage.getMarginAccount(req.params.userId, req.params.portfolioId);
    if (!marginAccount) {
...
router.post('/margin-accounts', async (req, res) => {
  try {
    const validatedData = insertMarginAccountSchema.parse(req.body);
    const marginAccount = await databaseStorage.createMarginAccount(validatedData);
...
router.get('/short-positions/user/:userId', async (req, res) => {
  try {
    const shortPositions = await databaseStorage.getUserShortPositions(req.params.userId);
...
router.get('/short-positions/portfolio/:portfolioId', async (req, res) => {
  try {
    const shortPositions = await databaseStorage.getPortfolioShortPositions(req.params.portfolioId);
    res.json(shortPositions);
  } catch (error) {
...
router.post('/short-positions', async (req, res) => {
  try {
    const validatedData = insertShortPositionSchema.parse(req.body);
    const shortPosition = await databaseStorage.createShortPosition(validatedData);
...
router.get('/npc-traders', async (req, res) => {
  try {
    const npcTraders = await databaseStorage.getActiveNpcTraders();
    res.json(npcTraders);
  } catch (error) {
...
router.get('/npc-traders/type/:traderType', async (req, res) => {
  try {
    const npcTraders = await databaseStorage.getNpcTradersByType(req.params.traderType);
    res.json(npcTraders);
  } catch (error) {
...
router.get('/npc-traders/firm/:firmId', async (req, res) => {
  try {
    const npcTraders = await databaseStorage.getNpcTradersByFirm(req.params.firmId);
    res.json(npcTraders);
  } catch (error) {
...
router.post('/npc-traders/:id/trading-decision', async (req, res) => {
  try {
    ...
    const trader = await databaseStorage.getNpcTrader(req.params.id);
    if (!trader) {
      return res.status(404).json({ error: 'NPC trader not found' });
    }
    
    const decision = NpcTradingEngine.calculateTradingDecision(
      trader, assetPrice, priceHistory, marketSentiment, volumeProfile
    );
...
router.post('/npc-traders', async (req, res) => {
  try {
    const validatedData = insertNpcTraderSchema.parse(req.body);
    const npcTrader = await databaseStorage.createNpcTrader(validatedData);
...
router.get('/information-tiers/:tierName', async (req, res) => {
  try {
    const tier = await databaseStorage.getInformationTier(req.params.tierName);
    if (!tier) {
...
router.get('/news/:userTier', async (req, res) => {
  try {
    ...
    const articles = await databaseStorage.getNewsArticlesByTier(userTier as 'elite' | 'pro' | 'free', limit);
    
    // Filter articles based on tier access
    const accessibleArticles = articles.filter(article => 
      InformationTierManager.hasNewsAccess(userTier as 'elite' | 'pro' | 'free', article)
    );
...
router.get('/news/asset/:assetId', async (req, res) => {
  try {
    const limit = req.query.limit ? parseInt(req.query.limit as string) : undefined;
    const articles = await databaseStorage.getNewsArticlesByAsset(req.params.assetId, limit);
    res.json(articles);
  } catch (error) {
...
router.post('/news', async (req, res) => {
  try {
    ...
    const article = await databaseStorage.createNewsArticle(validatedData);
...
router.get('/asset-mapping/:assetId', async (req, res) => {
  try {
    const mapping = await databaseStorage.getAssetFinancialMapping(req.params.assetId);
    if (!mapping) {
...
router.get('/asset-mapping/type/:instrumentType', async (req, res) => {
  try {
    const mappings = await databaseStorage.getAssetFinancialMappingsByType(req.params.instrumentType);
    res.json(mappings);
  } catch (error) {
...
router.post('/asset-mapping', async (req, res) => {
  try {
    const validatedData = insertAssetFinancialMappingSchema.parse(req.body);
    const mapping = await databaseStorage.createAssetFinancialMapping(validatedData);
