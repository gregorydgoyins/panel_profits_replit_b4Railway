...
async function main() {
  ...
  try {
    ...
    const charactersResult = await marvelExpansionService.expandCharacters(0, 100);
    ...
    const comicsResult = await marvelExpansionService.expandComics(0, 100);
    ...
    const creatorsResult = await marvelExpansionService.expandCreators(0, 50);
