...
import { tierClassificationService } from '../services/tierClassificationService';
...
for (const test of characterTests) {
  const classification = tierClassificationService.classifyCharacter(test.metadata);
  ...
  console.log(`\n${test.name}`);
  console.log(`  Tier: ${classification.tier} (${unifiedPricingEngine.getFranchiseTierInfo(classification.tier)})`);
...
for (const test of creatorTests) {
  const classification = tierClassificationService.classifyCreator(test.metadata);
  ...
  console.log(`\n${test.name}`);
  console.log(`  Tier: ${classification.tier} (${unifiedPricingEngine.getCreatorTierInfo(classification.tier)})`);
...
const erasToTest: Array<'golden' | 'silver' | 'bronze' | 'modern'> = ['golden', 'silver', 'bronze', 'modern'];
...
const supermanClass = tierClassificationService.classifyCharacter({
  name: 'Superman',
  appearances: 500
});
const supermanOriginal = unifiedPricingEngine.calculatePrice({
  assetType: 'character',
  name: 'Superman',
  era: 'golden',
...
const allStarClass = tierClassificationService.classifyCharacter({
  name: 'Superman (All-Star)',
  appearances: 12,
  isVariant: true
});
const supermanAllStar = unifiedPricingEngine.calculatePrice({
  assetType: 'character',
  name: 'Superman (All-Star)',
  era: 'modern',
...
const peterClass = tierClassificationService.classifyCharacter({
  name: 'Spider-Man',
  appearances: 450
});
const peterParker = unifiedPricingEngine.calculatePrice({
  assetType: 'character',
...
const milesClass = tierClassificationService.classifyCharacter({
  name: 'Miles Morales',
  appearances: 80
});
const milesMorales = unifiedPricingEngine.calculatePrice({
  assetType: 'character',
  name: 'Miles Morales',
  era: 'modern',
...
const stanClass = tierClassificationService.classifyCreator({
  name: 'Stan Lee',
  appearances: 5000,
  roles: ['writer', 'editorial']
});
const stanLee = unifiedPricingEngine.calculatePrice({
  assetType: 'creator',
  name: 'Stan Lee',
  era: 'golden',
...
const alexClass = tierClassificationService.classifyCreator({
  name: 'Alex Ross',
  appearances: 150,
  roles: ['artist', 'cover']
});
const alexRoss = unifiedPricingEngine.calculatePrice({
  assetType: 'creator',
  name: 'Alex Ross',
  era: 'modern',
