...
router.get('/assets/:assetId/similar', async (req, res) => {
  try {
    ...
    const similarAssets = await storage.findSimilarAssets(assetId, limit, threshold);
...
router.get('/recommendations/user/:userId', async (req, res) => {
  try {
    ...
    const result = await storage.getRecommendationsForUser(userId, limit);
...
router.get('/recommendations/portfolio/:portfolioId', async (req, res) => {
  try {
    ...
    const result = await storage.getPortfolioSimilarAssets(portfolioId, limit);
...
router.get('/comics/:gradingId/similar', async (req, res) => {
  try {
    ...
    const similarComics = await storage.findSimilarComicsByImage(gradingId, limit, threshold);
...
router.get('/insights/search', async (req, res) => {
  try {
    ...
    const searchResults = await storage.searchMarketInsightsByContent(query, limit, threshold);
...
router.get('/insights/:insightId/similar', async (req, res) => {
  try {
    ...
    const similarInsights = await storage.findSimilarMarketInsights(insightId, limit, threshold);
...
router.get('/patterns/:assetId/similar', async (req, res) => {
  try {
    ...
    const similarPatterns = await storage.findSimilarPricePatterns(assetId, timeframe, limit, threshold);
...
router.post('/embeddings/assets/:assetId', async (req, res) => {
  try {
    ...
    const updated = await storage.updateAssetEmbedding(assetId, embeddingResult.embedding);
...
router.post('/embeddings/comics/:gradingId', async (req, res) => {
  try {
    ...
    const grading = await storage.getComicGradingPrediction(gradingId);
    if (!grading) {
      return res.status(404).json({
        success: false,
    ...
    const embeddingResult = await vectorEmbeddingService.generateComicImageEmbedding(
      imageData,
      grading.imageName || undefined
    );
    ...
    const updated = await storage.updateComicImageEmbedding(gradingId, embeddingResult.embedding);
...
router.post('/indices/create', async (req, res) => {
  try {
    ...
    const created = await storage.createVectorIndices();
...
router.post('/indices/refresh', async (req, res) => {
  try {
    ...
    const refreshed = await storage.refreshVectorIndices();
...
router.get('/indices/status', async (req, res) => {
  try {
    ...
    const indexStatus = await storage.getVectorIndexStatus();
...
router.get('/status', async (req, res) => {
  try {
    const serviceStatus = vectorEmbeddingService.getServiceStatus();
