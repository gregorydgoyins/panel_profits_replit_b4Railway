...
export const SEVEN_HOUSES_CONFIG = [
  {
...
router.get('/api/seven-houses', async (req, res) => {
  try {
    ...
    if (!houses || houses.length === 0) {
      const seededHouses = [];
      for (const houseConfig of SEVEN_HOUSES_CONFIG) {
...
router.get('/api/seven-houses/rankings/power', async (req, res) => {
  try {
    const rankings = await storage.getHousePowerRankings();
    res.json(rankings);
  } catch (error) {
...
router.post('/api/seven-houses/rankings/update', isAuthenticated, async (req, res) => {
  try {
    ...
    const ranking = await storage.updateHousePowerRanking(houseId, changeAmount, reason);
    res.json(ranking);
  } catch (error) {
...
router.get('/api/seven-houses/events', async (req, res) => {
  try {
    ...
    const events = await storage.getHouseMarketEvents(limit);
    res.json(events);
  } catch (error) {
...
router.post('/api/seven-houses/events', isAuthenticated, async (req, res) => {
  try {
    ...
    const event = await storage.createHouseMarketEvent({
      affectedHouseId,
      eventType,
      description,
...
router.get('/api/seven-houses/:houseId/assets', async (req, res) => {
  try {
    const assets = await storage.getAssetsByHouse(req.params.houseId);
    res.json(assets);
  } catch (error) {
...
router.get('/api/seven-houses/:houseId/stats', async (req, res) => {
  try {
    const stats = await storage.getHouseStatistics(req.params.houseId);
    res.json(stats);
  } catch (error) {
