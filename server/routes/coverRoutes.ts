...
import { coverOrchestrator } from '../services/coverOrchestratorService';
...
router.get('/character/:name/first-appearance', async (req, res) => {
  try {
    ...
    const cover = await coverOrchestrator.getCharacterFirstAppearance(name);
...
router.get('/character/:name', async (req, res) => {
  try {
    ...
    const covers = await coverOrchestrator.getCharacterCovers(name, limit);
    res.json(covers);
  } catch (error) {
...
router.get('/key-issues', async (req, res) => {
  try {
    ...
    const covers = await coverOrchestrator.getKeyIssues(publisher, tier, limit);
    res.json(covers);
  } catch (error) {
...
router.get('/series/:series', async (req, res) => {
  try {
    ...
    const volumeYear = req.query.volumeYear ? parseInt(req.query.volumeYear as string) : undefined;
    const limit = req.query.limit ? parseInt(req.query.limit as string) : 100;
    
    const covers = await coverOrchestrator.getSeriesCovers(series, volumeYear, limit);
    res.json(covers);
  } catch (error) {
...
router.get('/tags', async (req, res) => {
  try {
    ...
    const covers = await coverOrchestrator.getCoversByTag(tags, limit);
    res.json(covers);
  } catch (error) {
...
router.post('/search', async (req, res) => {
  try {
    const covers = await coverOrchestrator.searchCovers(req.body);
    res.json(covers);
  } catch (error) {
...
router.get('/stats', async (req, res) => {
  try {
    const stats = await coverOrchestrator.getCoverStats();
    res.json(stats);
  } catch (error) {
...
router.get('/id/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const cover = await coverOrchestrator.getCoverById(id);
...
router.get('/issue', async (req, res) => {
  try {
    ...
    const cover = await coverOrchestrator.getCoverByIssue(
      publisher as string,
      series as string,
      issueNumber as string,
      volumeYear ? parseInt(volumeYear as string) : undefined,
      (variant as string) || 'regular'
    );
...
router.get('/issue/highest-grade', async (req, res) => {
  try {
    ...
    const cover = await coverOrchestrator.getHighestGradedCover(
      publisher as string,
      series as string,
      issueNumber as string,
      volumeYear ? parseInt(volumeYear as string) : undefined
...
router.get('/recent', async (req, res) => {
  try {
    ...
    const covers = await coverOrchestrator.getRecentlyCollected(limit);
    res.json(covers);
  } catch (error) {
