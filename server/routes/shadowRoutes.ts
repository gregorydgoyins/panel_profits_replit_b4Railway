...
import { z } from "zod";
import { shadowEconomyService } from "../services/shadowEconomyService";
...
const shadowTradeSchema = z.object({
  ...
  orderType: z.enum(['predatory', 'vampire', 'ghost'])
});

// Middleware to check corruption level
const requireCorruption = (minLevel: number) => {
...
router.get("/prices", isAuthenticated, requireCorruption(30), async (req: any, res) => {
  try {
    ...
    const shadowPrices = await shadowEconomyService.calculateBulkShadowPrices(
      realPrices,
      corruption
    );
...
router.post("/trade", isAuthenticated, requireCorruption(30), async (req: any, res) => {
  try {
    ...
    const availableOrders = shadowEconomyService.getAvailableShadowOrders(corruption);
    ...
    const shadowPriceData = shadowEconomyService.calculateShadowPrice(realPrice, corruption);

    // Execute the shadow trade
    const result = await shadowEconomyService.executeShadowTrade(
      userId,
      assetId,
      quantity,
      shadowPriceData.shadowPrice,
      realPrice,
      orderType,
      side
    );
...
router.get("/pools/:assetId", isAuthenticated, requireCorruption(30), async (req: any, res) => {
  try {
    const { assetId } = req.params;
    const corruption = req.userCorruption;

    // Get dark pool liquidity
    const pool = await shadowEconomyService.getDarkPoolLiquidity(assetId, corruption);
    ...
    const metrics = await shadowEconomyService.calculateDarkPoolMetrics(assetId);
...
router.get("/arbitrage", isAuthenticated, requireCorruption(30), async (req: any, res) => {
  try {
    const corruption = req.userCorruption;
    const minProfit = req.query.minProfit ? parseFloat(req.query.minProfit as string) : 5;

    const opportunities = await shadowEconomyService.findArbitrageOpportunities(
      corruption,
      minProfit
    );
...
router.get("/order-types", isAuthenticated, requireCorruption(30), async (req: any, res) => {
  try {
    const corruption = req.userCorruption;
    const availableOrders = shadowEconomyService.getAvailableShadowOrders(corruption);
...
router.get("/victims", isAuthenticated, requireCorruption(50), async (req: any, res) => {
  try {
    const userId = req.userId;
    const limit = req.query.limit ? parseInt(req.query.limit as string) : 10;
    
    const victims = await storage.getRecentVictims(userId, limit);
