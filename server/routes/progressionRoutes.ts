...
import { z } from "zod";
import {
  insertComicIssueVariantSchema,
  insertUserComicCollectionSchema,
  insertUserProgressionStatusSchema,
  insertHouseProgressionPathSchema,
  insertUserHouseProgressionSchema,
  insertTradingToolUnlockSchema,
  insertComicCollectionAchievementSchema,
  insertCollectionChallengeSchema,
  insertUserChallengeParticipationSchema
} from "@shared/schema";

const router = express.Router();
const progressionService = new ProgressionService(storage);
...
router.get("/status", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const status = await storage.getUserProgressionStatus(userId);
    res.json(status);
  } catch (error) {
...
router.post("/recalculate", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/collection", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const { sortBy = "acquiredAt", sortOrder = "desc", filterBy } = req.query;
    
    let collections = await storage.getUserComicCollections(userId);
...
router.post("/collection", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const data = insertUserComicCollectionSchema.parse({
      ...req.body,
      userId
    });
    
    const collection = await progressionService.addComicToCollection(
      userId, 
      data.variantId, 
      data
    );
...
router.put("/collection/:id", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const { id } = req.params;
    
    // Verify ownership
    const existing = await storage.getUserComicCollectionItem(id);
    ...
    const updated = await storage.updateUserComicCollection(id, updateData);
...
router.get("/variants", async (req, res) => {
  try {
    ...
    const filters = {
      ...
      primaryHouse: primaryHouse as string,
      minRarity: minRarity ? Number(minRarity) : undefined,
      maxPrice: maxPrice ? Number(maxPrice) : undefined,
      search: search as string
    };
    
    const variants = await storage.getComicIssueVariants(filters, Number(limit), Number(offset));
    res.json(variants);
  } catch (error) {
...
router.get("/variants/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const variant = await storage.getComicIssueVariant(id);
...
router.get("/houses", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const progressions = await storage.getUserHouseProgressions(userId);
...
router.get("/houses/:houseId", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const { houseId } = req.params;
    
    const progression = await storage.getUserHouseProgression(userId, houseId);
    if (!progression) {
    ...
    const progressionPaths = await storage.getHouseProgressionPaths(houseId);
...
router.get("/houses/paths/all", async (req, res) => {
  try {
    const paths = await storage.getAllHouseProgressionPaths();
    res.json(paths);
  } catch (error) {
...
router.get("/achievements/available", async (req, res) => {
  try {
    ...
    const achievements = await storage.getComicCollectionAchievements(filters);
    res.json(achievements);
  } catch (error) {
...
router.post("/achievements/check", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/trading-tools", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const { category, unlocked } = req.query;
    
    let tools = await storage.getTradingToolUnlocks(userId);
...
router.post("/trading-tools/check", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/challenges", async (req, res) => {
  try {
    ...
    const challenges = await storage.getCollectionChallenges(filters);
    res.json(challenges);
  } catch (error) {
...
router.get("/challenges/participation", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
    const { status, challengeType } = req.query;
    
    let participations = await storage.getUserChallengeParticipations(userId);
...
router.post("/challenges/:challengeId/participate", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/leaderboards", async (req, res) => {
  try {
    ...
    const leaderboard = await progressionService.getProgressionLeaderboard(
      category as string, 
      timeframe as string
    );
...
router.get("/leaderboards/rank", isAuthenticated, async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
router.get("/stats", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const [
      ...
      storage.getUserProgressionStatus(userId),
      storage.getUserComicCollections(userId),
      storage.getUserAchievements(userId),
      storage.getUserHouseProgressions(userId),
      storage.getTradingToolUnlocks(userId)
    ]);
      ...
      trading: {
        ...
        highestCategory: tradingUnlocks
          ...
          .sort((a, b) => {
            ...
            return categoryOrder[b.toolCategory as keyof typeof categoryOrder] - 
                   categoryOrder[a.toolCategory as keyof typeof categoryOrder];
