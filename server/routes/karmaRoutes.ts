...
import { karmicAlignmentService } from '../services/karmicAlignmentService.js';
import { storage } from '../storage.js';

const router = Router();

// GET /api/karma/alignment - Get user's current alignment data
router.get('/alignment', isAuthenticated, async (req, res) => {
  try {
...
router.post('/reveal-alignment', isAuthenticated, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'User not authenticated' });
    ...
    await karmicAlignmentService.recordKarmaAction(
      userId,
      'divine_intervention',
      {
...
router.get('/alignment-history/:timeframe?', isAuthenticated, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'User not authenticated' });
    ...
    const history = await karmicAlignmentService.getUserAlignmentHistory(userId, limit);
...
router.get('/profile', isAuthenticated, async (req, res) => {
  try {
    ...
    const profile = await karmicAlignmentService.getUserKarmicProfile(userId);
...
router.get('/consequences', isAuthenticated, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'User not authenticated' });
    ...
    const consequences = await karmicAlignmentService.getActiveTradingConsequences(userId);
...
router.post('/action', isAuthenticated, async (req, res) => {
  try {
    ...
    await karmicAlignmentService.recordKarmaAction(userId, actionType, context || {});
...
router.get('/summary', isAuthenticated, async (req, res) => {
  try {
    ...
    const profile = await karmicAlignmentService.getUserKarmicProfile(userId);
    const consequences = await karmicAlignmentService.getActiveTradingConsequences(userId);
    const recentHistory = await karmicAlignmentService.getUserAlignmentHistory(userId, 5);
