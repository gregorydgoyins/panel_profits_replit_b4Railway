...
- **Dashboard Widgets**: 19 professional-grade widgets for options/derivatives, market intelligence, advanced analytics, trading tools, and institutional flow, featuring a Bloomberg Terminal aesthetic with Watchmen noir styling. **Narrative Entity Widgets** ✅ **COMPLETE**: Three visual widget rows featuring Villains/Henchmen (blood-red theme), Sidekicks/Superheroes (baby blue/cyan dual-tone), and Locations/Gadgets (brown/goldenrod). Each widget displays franchise badges (top-right corner) showing publisher/universe (DC Comics, Marvel, etc.) with Hind font weight 300 typography. Widgets use 30-second polling intervals for data refresh, interactive cards with rimlight hover effects, and direct navigation to comprehensive bio detail pages. API endpoints return narrative_entities data with universe field for franchise distinction across all visualizations.
...
- **Ticker Nomenclature System**: Standardized dot-delimited symbol format (TICKR.V#.#ISSUE) for comic book issues enables professional-grade ticker display. Features centralized series abbreviation mapping (ASM→Amazing Spider-Man, BATM→Batman, DETC→Detective Comics, etc.), intelligent volume parsing (supports Vol, Vol., Volume, V# formats), issue number extraction, and auto-abbreviation fallback for unknown series. Integrated into Comic Vine and Marvel expansion services for consistent symbol generation across all new comic assets.
...
- **Price Update System**: Polling-based price updates using TanStack Query with 30-second refetch intervals. Stock ticker and dashboard widgets query `/api/market/ticker` and related endpoints every 30 seconds for fresh market data. This approach provides sufficient real-time feel for comic trading simulation without WebSocket complexity. **SCALE UPDATE**: Database now contains **401,666 total assets** with **394,140 current prices** (98.1% coverage). Asset breakdown: 20,429 Pinecone (99.8% priced), 23,272 Kaggle characters (100% priced), 2 Kaggle teams, remainder from entity seeding system. Market simulation engine updates prices in background.
